package com.feiniu.subAccount.util.json;
...
public class JsonNodeUtil {
    ...
    public static <T> T injectBean(String messageJson, Class[] classes, T object) {
        ...
        return injectBean(messageJson, fieldList, object);
    }
     ...
     * @return
     */
    public static <T> T getBean(String messageJson, Class<T> clazz, boolean hasPulledUpObject) {
        try {
            JsonNode jsonNode = JsonUtil.toNode(messageJson);
            T value = clazz.newInstance();
                ...
                if (methodName.startsWith("set")) {
                    ...
                    if (jsonNode.has(fieldName)) {
                        if (int.class.equals(returnType) || Integer.class.equals(returnType)) {
                            ...
                            parsedValue = getBean(currentFieldNode.toString(), returnType);
                        }
                    } else {
                        if (hasPulledUpObject) {
                            parsedValue = getBean(messageJson, returnType);
                        } else {
                            continue;
                        }
                    }
    ...
    public static <T> T getBean(String messageJson, Class<T> clazz) {
        return getBean(messageJson, clazz, false);
    }
