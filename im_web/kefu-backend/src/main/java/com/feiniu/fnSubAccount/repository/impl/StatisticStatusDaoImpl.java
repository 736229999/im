...
public class StatisticStatusDaoImpl implements StatisticStatusDao {
    ...
    public void clearAll(){
        ...
        sql = "DELETE FROM IM.IM_SUBMERCHANT_ATTENDANCE WHERE I_MERCHANT_ID="+"-1";
        session.createSQLQuery(sql).executeUpdate();

        sql = "DELETE FROM IM.IM_SUBMERCHANT_WORKLOAD WHERE I_MERCHANT_ID="+"-1";
        session.createSQLQuery(sql).executeUpdate();


        sql = "DELETE FROM IM.IM_MERCHANT_LANDING_DETAIL WHERE MERCHANT_ID="+"-1";
        session.createSQLQuery(sql).executeUpdate();


        sql = "DELETE FROM IM.IM_RECEPTION_MAP WHERE I_MERCHANT_ID="+"-1";
        session.createSQLQuery(sql).executeUpdate();
    }
}
