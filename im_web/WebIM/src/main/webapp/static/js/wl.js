...
(function () {
    ...
    Ttracker = {
        IComparer: function (a, b) {
            ...
            $.ajax({
                url: "http://diviner.jd.com/cookie?ck=" + tid + "." + sku, dataType: "jsonp", success: function (json) {
            ...
            $.ajax({
                url: "http://x.jd.com/aview?ck=" + tid + "." + sku, dataType: "jsonp", success: function (res) {
...
(function () {
    ...
    r.util = {
        join: function (l) {
            ...
            for (var g = [], t = ao.cookie[J](";"), l = RegExp("^\\s*" + y + "=\\s*(.*?)\\s*$"), s = 0; s < t[Q]; s++) {
                var p = t[s]["match"](l);
                p && g[N](p[1])
            }
            return g
    ...
    var al = ah(), ae = ah(), ag = ah(), I = ah(), d = ah(), aK = ah(), X = ah(), ap = ah(), af = ah(), ai = ah(), az = ah(), ay = ah(), aG = ah(), aQ = ah(), Z = ah(), U = ah(), B = ah(), z = ah(), M = ah(), aB = ah(), n = ah(), A = ah(), i = ah(), a = ah(), aN = ah(), aw = ah(), P = ah(), aL = ah(), f = ah(), au = ah(), c = ah(), aq = ah(), aU = ah(), b = ah(), ax = ah(), aO = ah(), aj = ah();
    var aP = function () {
        var y = {};
        ...
        this.set(z, t.D);
        this.set(M, t.C);
        this.set(aB, t.language);
        this.set(n, t.javaEnabled);
        this.set(A, t.characterSet);
        this.set(aL, an);
        this.set(aU, new Date().getTime());
        var s = r.util.Vv("pin");
        this.set(aO, s[Q] ? s[0] : "-");
        var p = "";
        if ((p = r.util.Vv("pinId")) && p[Q]) {
            p = p[0]
        } else {
            if ((p = r.util.Vv("TrackID")) && p[Q]) {
                p = p[0].split("|||");
                p = p[Q] > 1 ? p[1] : ""
            }
        }
    ...
    var an = "baidu:wd,baidu:word,so.com:q,so.360.cn:q,360so.com:q,360sou.com:q,baidu:q1,m.baidu:word,m.baidu:w,wap.soso:key,m.so:q,page.yicha:key,sz.roboo:q,i.easou:q,wap.sogou:keyword,google:q,soso:w,sogou:query,youdao:q,ucweb:keyword,ucweb:word,114so:kw,yahoo:p,yahoo:q,live:q,msn:q,bing:q,aol:query,aol:q,daum:q,eniro:search_word,naver:query,pchome:q,images.google:q,lycos:query,ask:q,netscape:query,cnn:query,about:terms,mamma:q,voila:rdata,virgilio:qs,alice:qs,yandex:text,najdi:q,seznam:q,search:q,wp:szukaj,onet:qt,szukacz:q,yam:k,kvasir:q,ozu:q,terra:query,rambler:query".split(","), aT = function () {
        ...
        var p = aJ();
        for (var l = p, g = ac.navigator, l = g.appName + g.version + l.language + g.platform + g.userAgent + l.javaEnabled + l.D + l.C + (ao.cookie ? ao.cookie : "") + (ao.referrer ? ao.referrer : ""), g = l.length, s = ac.history.length; s > 0;) {
            l += s-- ^ g++
        }
        return V(l)
    }, ab = function () {
        ...
        t.set(af, ao.location.hostname);
        t.set(ai, ao.title.replace(/\$/g, ""));
        t.set(az, ao.location.pathname);
        t.set(ay, ao.referrer.replace(/\$/g, ""));
        t.set(aG, ao.location.href);
        var g = r.util.Vv("__jda"), L = g[Q] > 0 ? g[0][J](".") : null;
        t.set(ae, L && !ar(L[1]) ? L[1] : Y());
        t.set(i, L ? L[2] : t.get(d));
        t.set(a, L ? L[3] : t.get(d));
        t.set(aN, L ? L[4] : t.get(d));
        t.set(aw, L ? L[5] : 1);
        ...
        var s = ao.location.href, C = ao.referrer, aW = aZ.get(I), y = r.util.getParameter(s, "utm_source"), t = [], O = aZ.get(f), L = aZ.get(au), K = aZ.get(c), g = r.util.Vv("__jdb")[Q] == 0;
        if (y) {
            var l = r.util.getParameter(s, "utm_campaign"), aY = r.util.getParameter(s, "utm_medium"), W = r.util.getParameter(s, "utm_term");
            t[N](y);
            t[N](l || "-");
            t[N](aY || "-");
            t[N](W || "not set");
            ...
            var p = C && C[J]("/")[2], aX = false;
            if (p && p[w](aW) < 0) {
                for (var aa = aZ.get(aL), ak = 0; ak < aa.length; ak++) {
                    var aV = aa[ak][J](":");
                    if (p[w](aV[0][H]()) > -1 && C[w]((aV[1] + "=")[H]()) > -1) {
                        var D = r.util.getParameter(C, aV[1]);
                        t[N](aV[0]);
                        t[N]("-");
                        t[N]("organic");
                        t[N](D || "not set");
        ...
        if (g || (!g && t[Q] > 0 && (t[0] !== O || t[1] !== L || t[2] !== K) && t[2] !== "referral")) {
            aZ.set(f, t[0] || aZ.get(f));
            aZ.set(au, t[1] || aZ.get(au));
            aZ.set(c, t[2] || aZ.get(c));
            aZ.set(aq, t[3] || aZ.get(aq));
            at(aZ)
        } else {
            h(aZ)
        }
        ...
        return [g.get(ag), g.get(ae) || "-", g.get(i) || "-", g.get(a) || "-", g.get(aN) || "-", g.get(aw) || 1][F](".")
    }, e = function (g) {
        return [g.get(ag), g.get(P) || 1, g.get(ae) + "|" + g.get(aw) || 1, g.get(aN) || g.get(d)][F](".")
    }, x = function (g) {
        return [g.get(ag), g.get(f) || ao.domain, g.get(au) || "(direct)", g.get(c) || "direct", g.get(aq) || "-"][F]("|")
    }, at = function (g) {
        var l = r.util.Vv("__jda");
        l.length == 0 ? E(g) : j(g, l[0])
    };
    var q = new aP(), av = function () {
        this.a = {};
        ...
        g.add("jdac", l.get(al)), g.add("jduid", l.get(ae)), g.add("jdsid", l.get(ae) + "|" + l.get(aw)), g.add("jdje", l.get(n)), g.add("jdsc", l.get(M)), g.add("jdsr", l.get(z)), g.add("jdul", l.get(aB)), g.add("jdcs", l.get(A)), g.add("jddt", l.get(ai) || "-"), g.add("jdmr", aC(l.get(ay))), g.add("jdhn", l.get(af) || "-"), g.add("jdfl", l.get(aQ)), g.add("jdos", l.get(B)), g.add("jdbr", l.get(Z)), g.add("jdbv", l.get(U)), g.add("jdwb", l.get(i)), g.add("jdxb", l.get(a)), g.add("jdyb", l.get(aN)), g.add("jdzb", l.get(aw)), g.add("jdcb", l.get(P)), g.add("jdusc", l.get(f) || "direct"), g.add("jducp", l.get(au) || "-"), g.add("jdumd", l.get(c) || "-"), g.add("jduct", l.get(aq) || "-"), g.add("jdlt", typeof jdpts != "object" ? 0 : jdpts._st == undefined ? 0 : l.get(aU) - jdpts._st), g.add("jdtad", l.get(b)), g.add("jdak", l.get(ax)), g.add("pinid", l.get(aj))
    }, aS = function (l, g, p, s) {
        g.add("jdac", l.get(al)), g.add("jduid", l.get(ae)), g.add("jdsid", l.get(ae) + "|" + l.get(aw)), g.add("jdje", "-"), g.add("jdsc", "-"), g.add("jdsr", "-"), g.add("jdul", "-"), g.add("jdcs", "-"), g.add("jddt", "-"), g.add("jdmr", aC(l.get(ay))), g.add("jdhn", "-"), g.add("jdfl", "-"), g.add("jdos", "-"), g.add("jdbr", "-"), g.add("jdbv", "-"), g.add("jdwb", "-"), g.add("jdxb", "-"), g.add("jdyb", "-"), g.add("jdzb", l.get(aw)), g.add("jdcb", s ? (aD() + s) : l.get(P)), g.add("jdusc", "-"), g.add("jducp", "-"), g.add("jdumd", "-"), g.add("jduct", "-"), g.add("jdlt", 0), g.add("jdtad", p), g.add("jdak", l.get(ax)), g.add("pinid", l.get(aj))
    }, aR = function () {
        aF(q) && aE(q);
    ...
    r.tracker = {
        sendOld: function (s, p, l, t) {
            var y = l && (l.jdac + "||" + l.jdje + "||" + l.jdsc + "||" + l.jdsr + "||" + l.jdul + "||" + l.jdcs + "||" + aC(l.jddt) + "||" + l.jdhn + "||" + l.jdfl + "||" + l.jdos + "||" + l.jdbr + "||" + l.jdbv + "||" + l.jdwb + "||" + l.jdxb + "||" + l.jdyb + "||" + l.jdzb + "||" + l.jdcb + "||" + l.jdusc + "||" + l.jducp + "||" + l.jdumd + "||" + l.jduct + "||" + l.jdlt + "||" + l.jdtad);
            if ((q.get(ae) % 100) === 1 && y) {
                y += "||" + aC(l.pinid)
            }
            var g = ("https:" == document.location.protocol ? "https://cscssl" : "http://csc") + ".jd.com/log.ashx?type1=" + aC(s) + "&type2=" + aC(p) + "&pin=" + aC(q.get(aO)) + "&uuid=" + l.jduid + "&sid=" + l.jdsid + (l.jdak ? ("&utmp=" + document.location.href + aC("&clickid=" + l.jdak)) : "") + "&referrer=" + aC(l.jdmr || "-") + "&jinfo=" + y + "&data=" + aC(t) + "&callback=?";
            $.getJSON(g, function () {
            })
        }, sendNew: function (l, s) {
            var p = aA();
            var g = ("https:" == document.location.protocol ? "https://mercuryssl" : "http://mercury") + ".jd.com/log.gif?t=" + l + "&m=" + q.get(al) + "&pin=" + aC(q.get(aO)) + "&uid=" + p.jduid + "&sid=" + p.jdsid + (p.jdak ? ("&cul=" + document.location.href + aC("&clickid=" + p.jdak)) : "") + "&v=" + aC(s) + "&ref=" + aC(ao.referrer) + "&rm=" + (new Date).getTime();
            var y = new Image(1, 1);
            y.src = g;
            y.onload = function () {
                y.onload = null;
            ...
            this.sendNew(l, p)
        }, ngloaderJSON: function (g, l) {
            if ((q.get(ae) % 100) === 1) {
                l.pinid = q.get(aj)
            }
            this.sendNew(g, $.toJSON(l))
    ...
    if ((window.pageConfig && window.pageConfig.product && window.pageConfig.product.cat) || S) {
        r.tracker.ngloader("item.010001", {
            sku: S[4] || window.pageConfig.product.skuid,
            cid1: S[1] || window.pageConfig.product.cat[0],
            cid2: S[2] || window.pageConfig.product.cat[1],
            cid3: S[3] || window.pageConfig.product.cat[2],
            brand: S ? "0" : window.pageConfig.product.brand
        })
    }
    (function () {
        if (isChecked()) {
...
function logJSON(b, a, c) {
    if (!JA) {
        return !1
    }
    JA.tracker.aloadingJSON(b, a, c);
    JA.tracker.ngloaderJSON("other.000000", {t1: b, t2: a, p0: c})
}
(function () {
