...
function setAutoLogin(v){
    setTimeout(function(){
        if($("#loginname").val() && v){
            if (encodeURIComponent($("#loginname").val(),'UTF-8')===v) {
                $("input[name='chkRememberMe']").attr("checked", true);
            }else{
...
function setALCookie(){
    if($("input[name='chkRememberMe']").attr("checked")){
        var Days = 3*30;
        setCookie('alpin',$("#loginname").val(),Days);
    }else{
        var v = cookie('alpin');
        if(v){
            setCookie('alpin','', -100);
...
if($("input[name='chkRememberMe']").length>0){
    var v = cookie('alpin');
    if(v){
        setAutoLogin(v);
