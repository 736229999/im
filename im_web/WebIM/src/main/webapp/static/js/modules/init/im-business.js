/**
 * Created by chenqing on 2016/2/19.
 */
seajs.use(['modules/feiniu', 'modules/util/generalServiceHelper', 'modules/util/generalUtil', 'JSON'],
    function (feiniu, serviceHelper, generalUtil, JSON) {
                ...
                $.post(self.config.SERVER,
                    ...
                    function (result){
                        if (result.success) {
                            ...
                            $(document).find('#J_staff_group_nickname').html(staffInfo.groupName+" - "+staffInfo.nickName);
                        } else {
                            $(document).find('#J_staff_group_nickname').html("获取飞牛员工信息失败~");
                        }
                    },
                    'json'
                );
        ...
        (function () {
            im_business.init.call(im_business);
        })();
