...
XIAOMI.namespace("app.analytics"),
    XIAOMI.app.analytics = function (element, options) {
        ...
        for (var _this = this,
                 isUrl = _this.options.url ? !0 : !1, isBDurl = _this.options.BDurl ? !0 : !1, bindFunction = function (links) {
                $.each(links,
                    function (k, v) {
                        var mstParamString = isUrl ? $(v).attr("href") : XIAOMI.lang.trim($(v).text()),
                            ...
                            mstParmas = "'" + _this.options.mstSite + "-" + _this.options.mstPage + "-" + _this.options.mstArea + "-" + _this.options.mstPosition[i] + paramIndex + "', '" + mstParamString;
    ...
    XIAOMI.app.dmFun = {
        ...
        getDmSys: function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    return _this.inTheQueue ? isOver === !0 ? (_this.stopQueue(), alert("抱歉，商品已售罄！"), ("2144400007" === _this.config.sku || "2141700014" === _this.config.sku || "2144100002" === _this.config.sku) && _this.config.callback("_hdover", _this.config.obj), !1) : (_this.getDmTimer && clearTimeout(_this.getDmTimer), void(token ? _this.getShopCart(token) : (_this.inTheQueue || _this.startQueue(), _this.getDmTimer = setTimeout(function () {
                            _this.getDmSys()
                        },
                        speed)))) : !1
                }
        ...
        stopQueue: function () {
            $("#xmDmQueue").modal("hide"),
                this.inTheQueue = !1,
            this.getDmTimer && clearTimeout(this.getDmTimer),
    ...
    XIAOMI.app.History = function () {
        ...
        for (var lens = historyVal.length - 1,
            ...
            itemArray = historyVal[i].split(","),
                goodsUrl = XIAOMI.GLOBAL_CONFIG.itemSite + "/" + itemArray[3] + ".html";
            var pro = {};
    ...
    XIAOMI.app.miniCart = {
        ...
        close: function () {
            ...
            _this.elmCartBtn.removeClass("mini-cart-on"),
                _this.elmCartList.html(this.loadingStr).hide()
        },
        ...
        formatData: function (data) {
            ...
            if (0 === data.errorno && data.totalItem > 0) {
                var lens = data.items.length,
                    goodsUrl = null,
                    listHtml = "<ul>",
                    countHtml = '<div class="count clearfix"><span class="total">共计 <em>' + data.total + "</em> 件商品<strong>合计：<em>" + data.totalPrice + '元</em></strong></span><a href="' + XIAOMI.GLOBAL_CONFIG.cartSite + '/cart"  class="btn btn-primary">去购物车结算</a></div>';
                if (data.totalItem > 5) {
                    var listHeight = 335;
    ...
    XIAOMI.app.miniLogin.prototype = {
        ...
        init: function () {
            var _this = this;
            -1 !== window.location.pathname.indexOf("cart") && XIAOMI.app.cookie("userId") && !XIAOMI.app.cookie("serviceToken") && _this._proxyiframe(),
        ...
        function bindChildrenList() {
            ...
            $categoryList.find(".nav-category-list").children("li").on({
                ...
                mouseleave: function () {
                    var $this = $(this);
                    window.clearTimeout(timeoutChildrenList),
                        timeoutChildrenList = window.setTimeout(function () {
    ...
    XIAOMI.app.carousel = function () {
        function init(elm, opt) {
            ...
            return 0 === $elm.length ? $elm : $elm.length > 1 ? ($elm.each(function () {
                ...
                }), void $btnNext.off("click.carousel").on("click.carousel",
                function (e) {
                    e.preventDefault(),
    ...
    XIAOMI.app.search = {
        ...
        init: function () {
            ...
            _this.inputObj.data("search-config") && (searchConfig = $.parseJSON(_this.inputObj.data("search-config").replace(/'/g, '"'))),
                $searchForm.append($('<div id="J_keywordList" class="keyword-list hide"><ul class="result-list"></ul></div>')),
                _this.inputObj.on({
                    focus: function () {
                        window.clearTimeout(timeoutFocus),
                            $("#J_keywordList").css("opacity", 1),
                            $searchForm.addClass("search-form-focus"),
                        !$(this).val() && searchConfig.defaultWords && _this.formatData(searchConfig.defaultWords)
                    },
                    blur: function () {
                        $("#J_keywordList").css("opacity", 0),
                            timeoutFocus = window.setTimeout(function () {
                                    $searchForm.removeClass("search-form-focus")
                                },
                                200)
                    },
                    keyup: function (e) {
                        $.trim($(this).val()).length <= 0 && searchConfig.defaultWords && (_this.keyword = null, _this.formatData(searchConfig.defaultWords)),
                            _this.focusMove(e.which),
                            clearTimeout(timeOut),
                        40 !== e.which && 38 !== e.which && (timeOut = setTimeout(function () {
                ...
                $searchForm.on("mouseenter", "#J_keywordList li",
                    function () {
        ...
        submit: function () {
            var _this = this;
            $("#J_searchForm").on("submit",
                function () {
                    var val = $.trim(_this.inputObj.val());
                    return "" !== val && (val = val.replace(/[\-_'=+|\\\/]/g, " "), window.location.href = "http://search.mi.com/search_" + encodeURIComponent(val)),
                        !1
                })
        }
    },
    ...
    XIAOMI.app.setLoginInfo = {
        ...
        createScript: function (uid) {
            ...
            script.src = "https://account.xiaomi.com/pass/userInfoJsonP?userId=" + uid + "&callback=setLoginInfo_getAccountInfo",
        ...
        updateDom: function (uid) {
            ...
            var timeoutUserMenu, html = '欢迎您 <b class="user-name">' + displayName + '<span class="arrow"></span></b><span class="sep">|</span><a href="' + this.config.orderSite + "/user/order?r=" + randomStr + '">我的订单</a><span class="sep">|</span><a href="http://account.xiaomi.com/" target="_blank">我的小米账户</a>',
...
var setLoginInfo_getAccountInfo = function (data) {
    ...
    if (data.userId) {
        ...
        XIAOMI.app.cookie("XM_" + _this.miid + "_UN", _this.data.userName, option),
            _this.updateDom(data.userId)
    }
};
