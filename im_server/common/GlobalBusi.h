...
class CMemCBusi
{
	public:
		...
		int GetTUserInfoEx(uint32 userId,tagGSRV_TUSERINFO_ACK_EX &TUserInfo);
		...
		bool AddLogoutLock(uint32 userId);
		bool DelLogoutLock(uint32 userId);
		bool GetLogoutLock(uint32 userId);
		...
		bool setExpireUserinfo(uint32 userid,uint32 timeout = 0);
		
		bool ModiUserIpPortInfo(const uint32 auid,uint32 ip,uint16 port);

//		bool SetServerMagic();
		
		bool DelDeviceUserInfo(uint32 nUserId,uint32 deviceId,uint32& count);
...
//		uint32 UpdateServerMagic(uint32 &time);
	...
	// 查询一台服务器上所有的用户
	int GetAllUser(int nServerNo, std::vector<tagGSRV_TUSERINFO_ACK_EX> &vecUser);
#ifdef IM_USE_MULTIDEVICE		
	bool AddDeviceUserInfo(const shared_ptr<CUser> &pUser);
	bool AddModiDeviceIpPortInfo(uint32 id,uint32 deviceId,uint32 ip,uint16 port);
	bool AddModiDeviceHeartInfo(uint32 id,uint32 deviceId,uint32 timestamp);
	bool DelDeviceUserInfo(uint32 nUserId,uint32 deviceId);
	bool GetAllDeviceUserInfo(uint32 userid,vector<multiDevice_t> &deviceInfo);
	bool GetDeviceUserInfo(uint32 userid,uint32 deviceId,multiDevice_t &deviceInfo);
	bool GetDeviceUserCount(uint32 userid, uint32 &count);	
	bool expireDeviceList(uint32 id);
#endif	
	// 修改用户签名
	bool ModiUserSign(uint32 userid,const char *pUserSign);
	// 更新在线时长
	bool ModiNewLiveTime(uint32 userid, uint32 newlivetime);
	// 修改用户的部分信息
	bool ModiPartUserInfo(uint32 userid, const STRU_PART_USERINFO &struPartUserInfo);
	// 修改用户登录时间
	bool ModiUserLoginTime(uint32 userid, uint32 logintime);
	// 获取排队列表
	bool GetCusterList(uint32 userid, vector<uint32> &vecList);
...
public:
	...
	//删除用户全局资料
	int DelUserInfo(const uint32 * auid,uint8 & count,uint32 * deviceID = NULL);
...
//	int QueryCSQueueInfo(const uint32 * auid,uint8 & count,CsQueue* csQueue);	
	...
	int setExpireUserinfo(uint32 userid,uint32 timeout = 0);
	// 从redis中获取用户全量信息
	int GetFUserInfo(uint32 userid, CUser &user);
	...
	bool GetCusterList(uint32 userid, vector<uint32> &vecList);
#ifdef IM_USE_MULTIDEVICE
	int QueryUserDeviceInfo(const uint32 auid,const uint32 deviceid,vector<multiDevice_t> &deviceinfo);
	int GetDeviceUserCount(uint32 userid, uint32 &count);
	bool DeviceUserOnline(uint32 userid, uint32 ip,uint16 port ,multiDevice_t *deviceInfo = NULL);
	int QueryUserDeviceInfo(const uint32 auid,const uint32 deviceid,multiDevice_t &deviceinfo);
	int AddModiDeviceUserInfo(const shared_ptr<CUser> &pUser);
	int DelDeviceUserInfo(uint32, uint32);	
	int AddModiDeviceHeartInfo(uint32 id,uint32 deviceId,uint32 timestamp);		
	int AddModiDeviceIpPortInfo(uint32 id,uint32 deviceId,uint32 ip,uint16 port);
