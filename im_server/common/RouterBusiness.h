...
public:
	...
	// 设置用户管理类
	void SetUserManager(CUserManager *pUserManager)
	{
	...
	// 获取用户管理类
	CUserManager* GetUserManager()	{	return m_pUserManager;	}

	
	//设置全局信息查询服务器
#ifdef IM_USE_CACHE
	void SetGlobalBusi(CGlobalBusi* pGlobalBusi) 
	{
		m_pGlobalBusi		= pGlobalBusi;
		m_pGlobalFamilyBusi = pGlobalBusi;
	}
#else
