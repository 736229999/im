...
public:
	...
	GlobalBusiControl(list<uint32> globalSrvIP,uint16 globalSrvPort);
	...
	void InitHeadMulCast(LPXT_HEAD lpHead,uint16 cmd,uint16 len,uint8 dtype = XT_HEAD::DEST_TYPE_SERVER,uint32 did = 0)
	{
		lpHead->cmd = cmd;
		lpHead->did = did;
		lpHead->dtype = dtype;
	...
	void SendAckData(SendDataStruct & sendData,uint32 dstIp);
	void SendMultiRequestData(SendDataStruct & sendData);
