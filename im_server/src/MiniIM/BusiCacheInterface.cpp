...
{
    uint8 count = 1;
    CLog::Log("GetCSSubAccount",CLog::TYPE_IMPORTANT,"1更新客服信息 %u !",clientService->m_nID);
    CSinfo_t csinfo;

    sprintf(csinfo.m_Csname,"%s",clientService->m_CSname);
    csinfo.m_CurConsult = clientService->m_CurConsult;      //增加咨询量
    ...
    csinfo.m_Permission = clientService->m_Permission;
    csinfo.m_Customerid.clear();
    map<uint32,customer_t>::iterator mappedIter;
    for(mappedIter = clientService->m_CSCustomerlist.begin(); mappedIter != clientService->m_CSCustomerlist.end();
        mappedIter++)
    {
        customer_t pFriendInfo = (mappedIter->second);
        CLog::Log("GetCSSubAccount", CLog::TYPE_IMPORTANT,"当前组 %u  有用户 %u",
                  pFriendInfo.teamid,
                  pFriendInfo.userid,
...
{
    uint8 count = 1;
    CLog::Log("CMemcacheUpdateConsultCSInfo",CLog::TYPE_IMPORTANT,"2更新客服信息 %u !",userInfo.m_nID);
    CSinfo_t csinfo;

    sprintf(csinfo.m_Csname,"%s",userInfo.m_Csname);
    csinfo.m_CurConsult = userInfo.m_CurConsult;        //增加咨询量
    csinfo.m_CurReception = userInfo.m_CurReception;
    csinfo.m_MaxRecConsult = userInfo.m_MaxRecConsult;
    csinfo.m_nID = userInfo.m_nID;
    csinfo.m_nChattingBuyerID = userInfo.m_nChattingBuyerID;
    csinfo.m_nDirectorID = userInfo.m_nDirectorID;
    csinfo.m_State = userInfo.m_State;
    csinfo.m_Permission = userInfo.m_State;
    csinfo.m_Customerid.clear();
#if 0
    vector<uint32>::iterator CustomeridIter;

    for(CustomeridIter = userInfo.m_Customerid.begin(); CustomeridIter != userInfo.m_Customerid.end();
        CustomeridIter++)
    {
        csinfo.m_Customerid.push_back(*CustomeridIter);
    }
#endif
    csinfo.m_Customerid = userInfo.m_Customerid;
