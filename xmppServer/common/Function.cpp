...
*/
//base64编码
std::string Base64Encode( const char* szInput)
{
    string strInput(szInput);
    return Base64Encode(strInput);
}
//base64编码
...
bool moumou_ntoa(uint32 ip,char *dest)
{
	if(dest == NULL)
	{
		return false;
	...
	daddr.s_addr = (in_addr_t)ip;
	daddr_len = strlen(inet_ntoa(daddr));	
	if(daddr_len <=15)     //IP址的字符串最多为15位	
	{
