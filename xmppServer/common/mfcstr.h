...
public:
	...
	{
		char* pEnd,*pCurrent;
		pEnd = pCurrent = (char*)(m_str.c_str() + m_str.length()-1);
		...
		while( *pCurrent == ' ' || 
			...
			*pCurrent == '\t' )
		{
			if( pCurrent <= m_str.c_str() )
			{
				break;
			}
			--pCurrent;
		}
		if(m_str.length() - (pEnd -pCurrent )>=0 && pEnd -pCurrent>0)
		{
			m_str.erase(m_str.begin()+ m_str.length() - (pEnd -pCurrent ),m_str.end());	
		}
	}
	void TrimLeft( )
	{   
		char* pBegin,*pCurrent;
		pBegin = pCurrent = (char*)m_str.c_str();
		...
		m_str.erase(m_str.begin(),m_str.begin()+(pCurrent-pBegin));
