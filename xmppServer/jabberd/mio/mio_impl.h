...
static void _mio_accept(mio_t m, mio_fd_t fd)
{
    ...
    mio_debug(ZONE, "new_cplus socket accepted fd #%d, %s:%d", newfd, ip, j_inet_getport(&serv_addr));

    /* set up the entry for this new_cplus socket */
    ...
    /* tell the app about the new_cplus socket, if they reject it clean up */
...
static void _mio_run(mio_t m, int timeout)
{
    ...
    MIO_ITERATE_RESULTS(m, retval, iter)
    {
        ...
        /* new_cplus conns on a listen socket */
...
static mio_fd_t _mio_listen(mio_t m, int port, const char *sourceip, mio_handler_t app, void *arg)
{
    ...
    /* by default we read for new_cplus sockets */
    mio_read(m,mio_fd);
