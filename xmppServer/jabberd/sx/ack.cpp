...
#define START_NODE		"<stream:features>"
#define SASL_BIND_NODE	"<bind xmlns='urn:ietf:params:xml:ns:xmpp-bind'/><session xmlns='urn:ietf:params:xml:ns:xmpp-session'/>"
#define COMPRESS_NODE	"<compression xmlns='http://jabber.org/features/compress'><method>zlib</method></compression>"
#define MECHANISMS_NODE "<mechanisms xmlns='urn:ietf:params:xml:ns:xmpp-sasl'><mechanism>PLAIN</mechanism><mechanism>ANONYMOUS</mechanism></mechanisms>"
#define AUTH_NODE		"<auth xmlns='http://jabber.org/features/iq-auth'/>"
#define REGISTER_NODE	"<register xmlns='http://jabber.org/features/iq-register'/>"
#define TLS_NODE		"<starttls xmlns='urn:ietf:params:xml:ns: xmpp-tls'></starttls>"
#define END_NODE		"</stream:features>"
...
static void _sx_ack_header(sx_t s, sx_plugin_t p, sx_buf_t buf) {
	...
	string ackHead;
	ackHead += START_NODE;
	ackHead += MECHANISMS_NODE;
	ackHead += AUTH_NODE;
	ackHead += REGISTER_NODE;
	if(s->has_reset == 1)/* 为1的话，有SASL_BIND_NODE */
	{		
		
		ackHead += SASL_BIND_NODE;
	}	
	if(s->compressed == 0)	//为0的话有COMPRESS_NODE
	{	
    	log_debug(ZONE, "s->compressed == 1");
		ackHead += COMPRESS_NODE;		
	}

	ackHead += END_NODE;
	
    log_debug(ZONE, "hacking ack namespace decl onto stream header %s",ackHead.c_str());
    /* get enough space */
    _sx_buffer_alloc_margin(buf, 0, strlen(STREAM_ACK_NS_DECL) + 2 + ackHead.length());

    /* overwrite the trailing ">" with a decl followed by a new_cplus ">" */
    memcpy(&buf->data[buf->len - 1], STREAM_ACK_NS_DECL ">", strlen(STREAM_ACK_NS_DECL)+1);
    buf->len += strlen(STREAM_ACK_NS_DECL);

	memcpy(&buf->data[buf->len], ackHead.c_str(), ackHead.length());
	buf->len += ackHead.length();	
}
