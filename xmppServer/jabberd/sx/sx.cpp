...
sx_t sx_new(sx_env_t env, int tag, sx_callback_t cb, void *arg) {
    ...
    XML_SetEntityDeclHandler(s->expat, (XML_EntityDeclHandler) _sx_entity_declaration);
#else
    ...
    if(env != NULL) {
        ...
        for(i = 0; i < env->nplugins; i++)
            if(env->plugins[i]->new_cplus != NULL)
                (env->plugins[i]->new_cplus)(s, env->plugins[i]);
    }

    _sx_debug(ZONE, "allocated new_cplus sx for %d", tag);
...
void sx_free(sx_t s) {
    ...
    while((buf = (sx_buf_t)jqueue_pull(s->wbufq)) != NULL)
        _sx_buffer_free(buf);
    if (s->wbufpending != NULL)
        _sx_buffer_free(s->wbufpending);

    while((nad =(nad_t) jqueue_pull(s->rnadq)) != NULL)
        nad_free(nad);
...
void _sx_reset(sx_t s) {
     ...
     * the caller (and others) hold references. so, we make a new_cplus sx_t,
     * copy the contents (only pointers), free it (which will free strings
     * and queues), then make another new_cplus one, and copy the contents back
     * into s */
    ...
    new_cplus = (sx_t) malloc(sizeof(struct _sx_st));
    memcpy(new_cplus, s, sizeof(struct _sx_st));
    sx_free(new_cplus);

    new_cplus = sx_new(NULL, temp.tag, temp.cb, temp.cb_arg);
    memcpy(s, new_cplus, sizeof(struct _sx_st));
    free(new_cplus);
...
/** utility: make a new_cplus buffer
...
void _sx_buffer_alloc_margin(sx_buf_t buf, int before, int after)
{
    ...
    if (buf->data == NULL || buf->len == 0) {
        if (buf->heap != NULL)
            buf->heap = (char *)realloc(buf->heap, before+after);
        else
            buf->heap =(char *) malloc(before+after);
    ...
    if (buf->heap != NULL) {
        ...
        if (old_leader >= before && old_leader <= (before * 4)) {
            buf->heap = (char *)realloc(buf->heap, before + buf->len + after);
            buf->data = buf->heap + old_leader;
            return;
        }
    }

    /* Most general case --- allocate a new_cplus buffer, copy stuff over, free the old one. */
