...
static void _sx_ssl_get_external_id(sx_t s, _sx_ssl_conn_t sc) {
	...
	/* Allocate new_cplus id-on-xmppAddr object. See rfc3921bis 15.2.1.2 */
...
static void _sx_ssl_client(sx_t s, sx_plugin_t p) {
    ...
    ctx = (SSL_CTX*)xhash_get((xht) p->priavte_cplus, s->req_from);
    if(ctx == NULL) {
        _sx_debug(ZONE, "using default ssl context for %d", s->tag);
        ctx = (SSL_CTX*)xhash_get((xht) p->priavte_cplus, "*");
    } else {
    ...
    if(pemfile != NULL) {
        ...
        /* load the priavte_cplus key */
        ret = SSL_use_PrivateKey_file(sc->ssl, pemfile, SSL_FILETYPE_PEM);
        if(ret != 1) {
            _sx_debug(ZONE, "couldn't load alternate priavte_cplus key from %s", pemfile);
            SSL_free(sc->ssl);
            free(sc);
            free(pemfile);
            return;
        ...
        /* check the priavte_cplus key matches the certificate */
        ret = SSL_check_private_key(sc->ssl);
        if(ret != 1) {
            _sx_debug(ZONE, "priavte_cplus key does not match certificate public key");
            SSL_free(sc->ssl);
            free(sc);
            free(pemfile);
            return;
...
static void _sx_ssl_server(sx_t s, sx_plugin_t p) {
    ...
    ctx = (SSL_CTX*)xhash_get((xht) p->priavte_cplus, s->req_to);
    if(ctx == NULL) {
        _sx_debug(ZONE, "using default ssl context for %d", s->tag);
        ctx = (SSL_CTX*)xhash_get((xht) p->priavte_cplus, "*");
    } else {
...
static void _sx_ssl_unload(sx_plugin_t p) {
    xht contexts = (xht) p->priavte_cplus;
    void *ctx;
...
int sx_ssl_server_addcert(sx_plugin_t p, const char *name, const char *pemfile, const char *cachain, int mode, const char *password) {
    ...
    if(ret != 1) {
        _sx_debug(ZONE, "couldn't load priavte_cplus key from %s; %s", pemfile, ERR_error_string(ERR_get_error(), NULL));
        SSL_CTX_free(ctx);
        return 1;
    }
    ...
    if(ret != 1) {
        _sx_debug(ZONE, "priavte_cplus key does not match certificate public key; %s", ERR_error_string(ERR_get_error(), NULL));
        SSL_CTX_free(ctx);
        return 1;
    }
