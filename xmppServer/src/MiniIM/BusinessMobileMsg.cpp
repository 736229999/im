...
/*	if(dbret == 1)
	{
		ack.ret = RESULT_FAILED;
		strcpy(ack.szError, "操作失败，请稍后再试!");
	}
	else if(dbret == 2)
	{
		ack.ret = RESULT_FAILED;
		strcpy(ack.szError, "用户未绑定手机!");
	}
	else if(dbret == 3)
	{
		ack.ret = RESULT_FAILED;
		strcpy(ack.szError, "对方手机号码错误!");
	}
	else if(dbret == 4)
	{
		ack.ret = RESULT_FAILED;
		strcpy(ack.szError,"发送成功，本月免费短信已用完，本条短信已扣0.1金币！");
	}
	else if(dbret == 5)
	{
		ack.ret = RESULT_FAILED;
		strcpy(ack.szError,"发送失败，本月免费短信已用完，金币余额不足，请登录http://pay.woku.com/充值");
	}
	else if(dbret == 6)
	{
		ack.ret = RESULT_FAILED;
		strcpy(ack.szError,"暂不支持联通号码！");
