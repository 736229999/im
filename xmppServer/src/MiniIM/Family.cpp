...
void CPermFamily::SendFamilyMsg(ST_OFFLINE_MSG* pOfflineMsg)
{
...
bool CTempFamily::DelMember(uint32 UserID, CGlobalBusi* pGlobalFamilyBusi)
{
	...
	{
		if ((*it) != UserID)
			familyGlobalFunc.TransitSetMemberMsg((*it), FAMILY_REQ_QUIT, UserID, m_FamilyID, TEMP_FAMILY, "");
	}
	...
	if (GetCurMemberNum() <= 2)
	{
			...
			it != m_FamilyMemberList.end(); ++it)
		{
			...
			familyGlobalFunc.TransitSetMemberMsg((*it), FAMILY_REQ_QUIT, (*it), m_FamilyID, TEMP_FAMILY, "");
		}
