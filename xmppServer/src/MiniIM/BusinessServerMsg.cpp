...
{ 
	...
	if (pUser && lpOnlineUser && lpOnlineUser->isDirect == 1)
		return RESULT_SUCCESS;

	if (pUser)
	{
		pUser->DelFromOnlineList();
		CAppManager::GetInstance().GetUserManager().DelHash(pUser->m_nID);
...
int CBusinessServerMsg::SendReply()
{
	...
	m_pAppNetwork->SendDataTo(pRecData->data,
							  pRecData->data_len);
	return RESULT_SUCCESS;
}
...
void CBusinessServerMsg::VerifyFriend()
{
	...
	CUser* pUser = CAppManager::GetInstance().GetUserManager().Find(pUserAuthReq->to_id);
...
void CBusinessServerMsg::P2PMessage()
{
	...
	CUser* pUser = CAppManager::GetInstance().GetUserManager().Find(pP2PMsgSend->Recver);
	if(pUser)
		return;
		...
		pP2PMsgSend->type == SERVER_P2PMSG_TYPE_MYWORD	)
	{
		...
		if(m_pAppNetwork->UserType(msg.from_id) == TYPE_WEB || m_pAppNetwork->UserType(msg.to_id) == TYPE_WEB)
		{}
		else
		{
			m_pRouter->GetDataBaseMini()->
				Save_Offline_Msg(pP2PMsgSend->Recver,
				pP2PMsgSend->Sender,
				0,
				0,
				(uint32)current,
				...
				m_RecvPacket.GetData(),
				m_RecvPacket.GetSize(),
				msg.data,msg.send_time,msg.uuid);
		}
	}
...
void CBusinessServerMsg::FamilyMsg()
{
	...
	else
	{
		...
		if (lpTransmitHead->origin_id &0x80000000 )
		{
			...
			if (pTempFamily)
				pTempFamily->DelMember(m_pRecvHead->did, m_pRouter->GetGlobalFamilyBusi());				
...
void CBusinessServerMsg::FamilyVerifyMsg()
{
	...
	CUser* pUser = CAppManager::GetInstance().GetUserManager().Find(m_pRecvHead->did);	//如果验证者不在线。保存数据库
	if(pUser )
	{	
...
{
	CUser* pUser = CAppManager::GetInstance().GetUserManager().Find(m_pRecvHead->did);	//接收通知者不在线
	if(pUser)
		return;
	else
	{
