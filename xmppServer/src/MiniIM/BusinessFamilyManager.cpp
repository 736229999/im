...
void CBusinessFamilyGlobalFunc::TransitSetMemberMsg(uint32 recvID, uint8 action, uint32 actionID, uint32 familyID, uint8 familyProp, const char* familyName, uint32 familyMsgID, uint32 nInviteID)
{
	int len = 0;
...
{
	LPXT_GET_CINFO_REQ lpReq;
	lpReq = (XT_GET_CINFO_REQ *)m_RecvPacket.GetData();
			...
			for(int i = 0; i < nMemNum; ++i)
 			{
				if (UStatus[i].id == pFamilyMem[i].nUserID)
					pFamily->AddMember(pFamilyMem[i].nUserID, pFamilyMem[i].nProp, UStatus[i].status);
				else
					pFamily->AddMember(pFamilyMem[i].nUserID, pFamilyMem[i].nProp, XTREAM_OFFLINE);
			}
		}
		//设置当前成员状态
	...
	{		
		...
		if (lpReq->nAction == CMEMBER_ADD)
		{
			...
			for(int i = 0; i < nRetCount; ++i)
			{
				...
				pUser = UserManager.Find(lpReq->nMemID[i]);
				if (pUser)				//用户在线
				{
				...
				for(; it != pFamily->m_FamilyMembers.end(); ++it)
				{
					...
					{
						familyGlobalFunc.TransitSetMemberMsg((*it).first, lpReq->nAction, lpReq->nMemID[i], pFamily->m_FamilyID, pFamily->m_FamilyProp, pFamily->m_FamilyName, pFamily->m_FamilyMaxMsgID);
					}
				}
		...
		else if (lpReq->nAction == CMEMBER_DEL)
		{
			...
			pUser = UserManager.Find(lpReq->nMemID[0]);
			if (pUser)				//用户在线
				pUser->Del_Perm_Family(pFamily->m_FamilyID);
			CBusinessFamilyGlobalFunc familyGlobalFunc;

			familyGlobalFunc.TransitSetMemberMsg(lpReq->nMemID[0], lpReq->nAction, lpReq->nMemID[0], pFamily->m_FamilyID, pFamily->m_FamilyProp, pFamily->m_FamilyName);
			...
			for(; it != pFamily->m_FamilyMembers.end(); ++it)
			{
				if ((*it).second.nStat != XTREAM_OFFLINE && (*it).first != m_pRecvHead->sid)
				{
					familyGlobalFunc.TransitSetMemberMsg((*it).first, lpReq->nAction, lpReq->nMemID[0], pFamily->m_FamilyID, pFamily->m_FamilyProp, pFamily->m_FamilyName);
				}
			}
		}
		...
		else if(lpReq->nAction < CMEMBER_REFUSE_IN_2 + 1)
		{
			...
			for(; it != pFamily->m_FamilyMembers.end(); ++it)
			{
				if ((*it).second.nStat != XTREAM_OFFLINE )
				{
					...
					{
						familyGlobalFunc.TransitSetMemberMsg((*it).first, lpReq->nAction, lpReq->nMemID[0], pFamily->m_FamilyID, pFamily->m_FamilyProp, pFamily->m_FamilyName,0,lpReq->nInviteID);
					}
				}
		...
		for( int i = 0; i < lpReq->nCount; ++i)
		{
			if (RESULT_SUCCESS == pFamily->AddMember(lpReq->nMemID[i]))
			{
				...
				for(; it != pFamily->m_FamilyMemberList.end(); ++it)
				{
					...
					{
						familyGlobalFunc.TransitSetMemberMsg((*it), lpReq->nAction, lpReq->nMemID[i], pFamily->m_FamilyID, pFamily->m_FamilyProp, szfamilyname);
					}
				}
			}
		}
...
int CBusinessFamilyApply::DoWork()
{
			...
			case FAMILY_JOIN_ANY:  // 不需要验证
				{
					...
					else
					{
						...
						CUser *pUser = CAppManager::GetInstance().GetUserManager().Find(Req.nUserID);
						if (pUser)	
							pUser->Add_Perm_Family(pFamily->m_FamilyID, pFamily->m_FamilyMaxMsgID);
						else		//非直连本服务器
							familyGlobalFunc.TransitSetMemberMsg(Req.nUserID, Req.nAction, Req.nUserID, pFamily->m_FamilyID, pFamily->m_FamilyProp, pFamily->m_FamilyName,pFamily->m_FamilyMaxMsgID);
						...
						if (!bReleaseFamily)
						{
							...
							for(; it != pFamily->m_FamilyMembers.end(); ++it)
							{
								if ((*it).second.nStat != XTREAM_OFFLINE && (*it).first != Req.nUserID)
									familyGlobalFunc.TransitSetMemberMsg((*it).first, Req.nAction, Req.nUserID, pFamily->m_FamilyID, pFamily->m_FamilyProp, pFamily->m_FamilyName);
							}
						}
	...
	else if (Req.nAction == FAMILY_REQ_QUIT)
	{
		...
		else if (Req.nFamilyProp == TEMP_FAMILY)
		{
			...
			pFamily->DelMember(Req.nUserID,m_pRouter->GetGlobalFamilyBusi());
			CBusinessFamilyGlobalFunc familyGlobalFunc;
			...
			if (pUser)	
				...
				familyGlobalFunc.TransitSetMemberMsg(Req.nUserID, Req.nAction, Req.nUserID, pFamily->m_FamilyID, pFamily->m_FamilyProp, "");
		}
	}
	else	//错误的操作类型
	{
...
int CBusinessFamilyVerify::DoWork()
{
	...
	else if (lpReq->nAction == FAMILY_JOIN_AGREE)
	{
		...
		else
		{
			...
			CUser* pUser = CAppManager::GetInstance().GetUserManager().Find(lpReq->nRequestID);
			if (pUser)
		...
		for(; it != pFamily->m_FamilyMembers.end(); ++it)
		{
			if ((*it).second.nStat != XTREAM_OFFLINE && (*it).first != lpReq->nManagerID && (*it).first != lpReq->nRequestID)	//同意加入方不用接收该消息
                familyGlobalFunc.TransitSetMemberMsg((*it).first, CMEMBER_ADD, lpReq->nRequestID, lpReq->nFamilyID,pFamily->m_FamilyProp,pFamily->m_FamilyName);
		}
...
void CBusinessFamilyVerify::TransitFamilyVerifyMsg(uint32 recvID, uint32 familyID, char* msg,int msgLen)
{
	...
	m_pAppNetwork->SendDataTo(sendData.data,sendData.data_len,SEND_RETRY_TIME);
...
// 									m_RecvPacket.GetData());
		...
		for(; it != pFamily->m_FamilyMembers.end(); ++it)
		{
			...
			{
				CUser* pUser = CAppManager::GetInstance().GetUserManager().Find(Req.senderID);
				if (pUser)
				{
...
void CBusinessFamilySendMsg::TransitFamilyMsg(uint32 recvID,uint32 familyID,uint32 msgID,char * msg,int msgLen, uint8 familyProp, int retrytime)
{
	...
	CAppManager::GetInstance().InitHead(lpSendHead,CMD_SERVERMSG_IND,len,recvID);
	sendData.data_len = len + (int)sizeof(XT_HEAD);
	m_pAppNetwork->SendDataTo(sendData.data,sendData.data_len,retrytime);

	CUser* pUser = CAppManager::GetInstance().GetUserManager().Find(recvID);
	if (pUser && familyProp == PERM_FAMILY)
...
int CBusinessFamiliyOfflineMsg::DoWork()
{
	...
	if (pFamily)
	{
		...
		for(int i = 0; i < nRetCount; ++i)
		{
			pFamily->SendFamilyMsg(&m_pFamilyMsgBuf[i]);
		}
