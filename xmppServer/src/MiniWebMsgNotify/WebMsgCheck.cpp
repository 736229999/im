...
{
	m_hMMT = timeSetEvent(3*TIMECHECKIOD,TIMECHECKIOD,MySystemLogProc,(void *)this,TIME_PERIODIC);
	if( NULL == m_hMMT )
	{
		CLog::Log("CWebMsgCheck",CLog::TYPE_ERROR,"网站消息定时器设置失败");
	}
}
...
{
	m_hMMT = timeSetEvent(2*TIMECHECKIOD,TIMECHECKIOD,MyOperationProc,(void *)this,TIME_PERIODIC);
	if( NULL == m_hMMT )
	{
		CLog::Log("COperationCheck",CLog::TYPE_ERROR,"网站消息定时器设置失败");
	}
}
