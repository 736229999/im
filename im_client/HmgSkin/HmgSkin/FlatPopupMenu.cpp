							...
							IN LPCTSTR lpTyp)
{
	...
	if ( !pImage->IsNull() && pImage->GetBPP() == 32 )
	{
...
CFlatPopupMenu::CFlatPopupMenu()
{
	...
	m_Colors[colorBorder]=/*GetSysColor(COLOR_MENUTEXT)*/RGB(196,196,196);
	m_Colors[colorBackground]=/*GetSysColor(COLOR_MENU)*/RGB(255,255,255);
	m_Colors[colorText]=/*GetSysColor(COLOR_MENUTEXT)*/0x4f4f4f;
	...
	m_Colors[colorIconArea]=/*GetSysColor(COLOR_MENUTEXT)*/RGB(236,238,237);
...
CFlatPopupMenu::~CFlatPopupMenu()
{
	for(int i = 0; i != m_vecpImg.size(); ++i)
	{
		if(!m_vecpImg[i]->IsNull())
			m_vecpImg[i]->Destroy();
		delete m_vecpImg[i];
		m_vecpImg[i] = NULL;
	}
	...
	for(size_t i = 0; i != m_vecpSubMenu.size(); ++i)
	{
		delete m_vecpSubMenu[i];
		m_vecpSubMenu[i] = NULL;
	}
	m_vecpSubMenu.clear();

	::UnregisterClass("fpmMenuClass", m_hInstance);
}
...
void CFlatPopupMenu::Cleanup(void)
{
	if(m_hFont)
	{
		...
		DeleteObject(m_hIcomBrush);
		DeleteObject(m_hSepPen);
...
void CFlatPopupMenu::CreateObjects(void)
{
	...
	m_hIcomBrush=CreateSolidBrush(m_Colors[colorIconArea]);
	m_hSepPen=CreatePen(PS_SOLID,1,m_Colors[colorIconArea]);
...
// 添加位图，位图高度必须相同
BOOL CFlatPopupMenu::AddImg(HINSTANCE hInstance,UINT nResID, LPCTSTR lpTyp)
{
	CImage *pimg = new CImage();
	if( !LoadImageFromResource(pimg, hInstance, nResID, lpTyp) )
		return FALSE;

	if(m_nImgHeight == -1)
	{
		m_nImgHeight = pimg->GetHeight();
	}

	if(pimg->GetHeight() != m_nImgHeight || pimg->GetWidth() % m_nImgHeight != 0)
		return FALSE;		// 高度宽度不被允许

	m_vecpImg.push_back(pimg);
	return TRUE;
}
...
/* Create it */
/*************/
bool CFlatPopupMenu::Create(HINSTANCE hInstance)
{
	// save variables
...
UINT CFlatPopupMenu::Track(int x,int y,HWND hWnd,const bool bModal,const bool bPopup, const bool bDisplayTop)
{
	HDC hDC;
	...
	m_Width=1+g_ItemHeight+maxw+g_ItemHeight+1;
	m_SelectedItem=-1;
	m_SelectedID=0;
	...
	if(y+top>rcScreen.bottom)
		y=max(0,rcScreen.bottom-(top));
	...
	::SetWindowPos(m_hWnd, CWnd::wndTopMost.m_hWnd,x,y,m_Width,top,SWP_SHOWWINDOW);
...
void CFlatPopupMenu::OnTimer(HWND hWnd,unsigned short timerid)
{
	...
	if(m_SelectedItem!=-1 && m_Items[m_SelectedItem].m_pPopup)
	{
		...
		p.x=m_Width + 2;
		p.y=m_Items[m_SelectedItem].m_Top;
...
void CFlatPopupMenu::DrawItem(HWND hWnd,HDC hDC,const int index,const CItem& item)
{
	...
	{
		DrawSeparator(hDC,index == 0 ? item.m_Top + 1 : item.m_Top);
		return;
	}
	...
	if(bSelected)
	{
		Rectangle(hDC,x+1,item.m_Top+1,m_Width-1,item.m_Top+g_ItemHeight-1);
	}
	else
	{
		Rectangle(hDC,x,item.m_Top,m_Width,item.m_Top+g_ItemHeight);
	}
	...
	if(!bSelected)
	{
		hOldBrush=SelectObject(hDC, m_hIcomBrush);
		hOldPen=SelectObject(hDC,GetStockObject(NULL_PEN));

		Rectangle(hDC,x,item.m_Top-(index==0?0:1),g_ItemHeight,item.m_Top+g_ItemHeight);
	...
	if(item.m_IconIndex!=-1 && m_vecpImg.size() > 0)
	{
		// 查找使用哪个img绘制
		CImage *pImg = NULL;
		int nImgX = 0;
		...
		for(int i = 0; i < m_vecpImg.size(); ++i)
		{
			CImage* &pimg = m_vecpImg[i];
			if(nIndexBgn + (pimg->GetWidth() / m_nImgHeight) > item.m_IconIndex)
			{
				nImgX = (item.m_IconIndex - nIndexBgn) * m_nImgHeight;
				pImg = pimg;
				break;
			}
			else
			{
				nIndexBgn += pimg->GetWidth() / m_nImgHeight;
			}
		}
		...
		if(pImg != NULL)
		{
			...
			int DestX = x;
			int DestY = item.m_Top;
			int DestW = g_ItemHeight;
			int DestH = g_ItemHeight;
			if(m_nImgHeight < g_ItemHeight)
			{
				DestX += (g_ItemHeight-m_nImgHeight)/2;
				DestY += (g_ItemHeight-m_nImgHeight)/2;
				DestW = DestH = m_nImgHeight;
			}

			pImg->Draw(hDC, DestX, DestY, DestW, DestH, nImgX, 0, m_nImgHeight, m_nImgHeight);
		}		
	}
	...
	rect.left=1+g_ItemHeight+2;
	rect.top=item.m_Top+((g_ItemHeight-h)/2);
	...
	if(item.m_pPopup)
	{
		...
		p[0].y=item.m_Top+item.m_Height/2;
		p[1].y=p[0].y+1;
...
void CFlatPopupMenu::DrawSeparator(HDC hDC,int top)
{
	...
	p[1].x=m_Width;
	p[0].y=p[1].y=top+2;
	hOldPen=SelectObject(hDC,m_hSepPen);
	Polyline(hDC,p,2);
	SelectObject(hDC,hOldPen);

	HGDIOBJ hOldBrush=SelectObject(hDC, m_hIcomBrush);
	hOldPen=SelectObject(hDC,GetStockObject(NULL_PEN));

	Rectangle(hDC,1,top-1,g_ItemHeight,top+4);
...
BOOL CFlatPopupMenu::AppendMenu(CMenu *pMenu)
{
	...
	for(int i = 0; i < nMenuCount; ++i)
	{
		...
		if(pSubMenu == NULL)
		{
			...
			{
				AppendItem(CFlatPopupMenu::itemSeparator,NULL,-1);
			}
			else
			{
				AppendItem(0, strMenuText, nId);
			}
		}
		else
		{
			CFlatPopupMenu *pTempMenu = AppendSubMenu(pSubMenu);
			AppendPopup(0, strMenuText, *pTempMenu);
		}
	}
...
CFlatPopupMenu* CFlatPopupMenu::AppendSubMenu(CMenu *pSubMenu)
{
	CFlatPopupMenu *pSubFlatMuen = new CFlatPopupMenu();
	pSubFlatMuen->Create(m_hInstance);
	m_vecpSubMenu.push_back(pSubFlatMuen);
	...
	for(int i = 0; i < nMenuCount; ++i)
	{
		...
		if(pTempSubMenu == NULL)
		{
			if(nId == 0 && strMenuText.IsEmpty())
			{
				pSubFlatMuen->AppendItem(CFlatPopupMenu::itemSeparator,NULL,-1);
			}
			else
			{
				pSubFlatMuen->AppendItem(0, strMenuText, nId);
			}
		}
		else
		{
			CFlatPopupMenu *pTempMenu = AppendSubMenu(pSubMenu);
			pSubFlatMuen->AppendPopup(0, strMenuText, *pTempMenu);
		}
	}
	return pSubFlatMuen;
}
...
/*获取条目数*/
int CFlatPopupMenu::GetItemCount()
{
	return m_Items.size();
}

/*设置条目的图片索引，从0开始，如果为-1，表示没图标*/
void CFlatPopupMenu::SetItemImgIndex(int nItemIndex, int nImageIndex)
{
	if(nItemIndex < 0 || nItemIndex >= m_Items.size())
		return;

	m_Items[nItemIndex].m_IconIndex = nImageIndex;
}

/*获取子菜单*/
CFlatPopupMenu* CFlatPopupMenu::GetSubMenu(int nItemIndex)
{
	if(nItemIndex < 0 || nItemIndex >= m_Items.size())
		return NULL;

	return m_Items[nItemIndex].m_pPopup;
}