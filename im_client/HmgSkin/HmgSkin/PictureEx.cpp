...
UINT CPictureEx::GetPictureFileSize() const
{
	return m_nPictureFileSize ;
}
...
BOOL CPictureEx::Load(LPCTSTR szFileName)
{
	...
	dwSize = (ULONGLONG)file.GetLength();
...
BOOL CPictureEx::Load(HINSTANCE hResourceHandle, LPCTSTR szResourceName, LPCTSTR szResourceType)
{
	ASSERT(szResourceName);
	ASSERT(szResourceType);

	HRSRC hPicture = FindResource(hResourceHandle,szResourceName,szResourceType);
	HGLOBAL hResData;
	if (!hPicture || !(hResData = LoadResource(hResourceHandle,hPicture)))
	{
		TRACE(_T("Load (resource): Error loading resource %s\n"),szResourceName);
		return FALSE;
	};
	DWORD dwSize = SizeofResource(hResourceHandle,hPicture);
...
void CPictureEx::OnPaint() 
{
	...
	if ( m_hMemDC )
	{
		float f1 = rcClient.Height()/(float)m_PictureSize.cy;
		float f2 = rcClient.Width()/(float)m_PictureSize.cx;
		...
		if ( f<1.0 )
		{
			stretch_width  = (int)(m_PictureSize.cx*f);
			stretch_height = m_PictureSize.cy*f;
		}
		else
		{
			stretch_width  = m_PictureSize.cx;
			stretch_height = m_PictureSize.cy;
		}

		stretch_x  = (rcClient.Width()-stretch_width)/2;
		stretch_y  = (rcClient.Height()-stretch_height)/2;

		//FillRect(dc.m_hDC,&rcClient,(HBRUSH)COLOR_WINDOW);
		dc.FillSolidRect(rcClient,m_clrBackground);

		CBitmap bmpPicture;
		bmpPicture.CreateCompatibleBitmap(&dc,m_PictureSize.cx,m_PictureSize.cy);

		CDC dcPicture;
		dcPicture.CreateCompatibleDC(&dc);

		CBitmap *pOldbmp = dcPicture.SelectObject(&bmpPicture);

		::BitBlt(dcPicture.m_hDC,0,0,m_PictureSize.cx,m_PictureSize.cy,m_hMemDC,0,0,SRCCOPY);

		Graphics graphics(dc.m_hDC);

		Bitmap *bitmap = Bitmap::FromHBITMAP((HBITMAP)bmpPicture.m_hObject,NULL);
		
		if ( bitmap )
		{
			Unit  units;

			RectF destRect((REAL)stretch_x,	(REAL)stretch_y,(REAL)stretch_width,(REAL)stretch_height),
				srcRect;
			bitmap->GetBounds(&srcRect, &units);
			graphics.DrawImage(bitmap, destRect, srcRect.X, srcRect.Y, srcRect.Width,	srcRect.Height,	UnitPixel, NULL);

			dcPicture.SelectObject(pOldbmp);

			delete bitmap;
		}
	//	::StretchBlt(dc.m_hDC,stretch_x,stretch_y,stretch_width,stretch_height,
