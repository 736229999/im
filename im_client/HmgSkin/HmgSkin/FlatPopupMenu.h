...
// 从资源中加载图片
AFX_EXT_CLASS BOOL LoadImageFromResource( IN CImage* pImage,
										  IN HINSTANCE hInstance,
										  IN UINT nResID, 
										  IN LPCTSTR lpTyp);

class AFX_EXT_CLASS CFlatPopupMenu
{
public:
	// data members
...
public:
	...
	// 添加位图，位图高度必须相同
	BOOL AddImg(HINSTANCE hInstance,UINT nResID, LPCTSTR lpTyp);
	...
	UINT Track(int x,int y,HWND hWnd,const bool bModal,const bool bPopup=false, const bool bDisplayTop = false);
	...
	/*获子菜单*/
	CFlatPopupMenu* GetSubMenu(int nItemIndex);

private:
	std::vector<CFlatPopupMenu*> m_vecpSubMenu;
};
