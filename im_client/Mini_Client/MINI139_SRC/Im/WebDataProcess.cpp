...
UINT CWebDataProcess::GetWebPictureListProc(LPVOID p)
{
	...
	static char sz139Date[] = "2005-1-1 00:00:00";
	...
	try
	{
		...
		if ( strcmp(pThread->szBeginDate,"")==0 )
			objWI.GetWebInterface()->raw_GetUserPictureList(atol((LPCSTR)result.strID),A2W(sz139Date),A2W(pThread->szEndDate),&retValue);
		else
		if ( strcmp(pThread->szBeginDate,"-")==0 )
			objWI.GetWebInterface()->raw_GetUserPictureList(atol((LPCSTR)result.strID),L"",A2W(pThread->szEndDate),&retValue);
		else
			objWI.GetWebInterface()->raw_GetUserPictureList(atol((LPCSTR)result.strID),A2W(pThread->szBeginDate),A2W(pThread->szEndDate),&retValue);
	
		result.strWebResult = retValue;
	}
	catch(...)
	{
		result.strWebResult = L"";
	}
...
UINT CWebDataProcess::GetWebArticleListProc(LPVOID p)
{
	...
	try
	{
		...
		if ( strcmp(pThread->szBeginDate,"")==0 )
			objWI.GetWebInterface()->raw_GetUserArticleList( atol((LPCSTR)pThread->strID),A2W(sz139Date),A2W(pThread->szEndDate),&retValue);
		else
		if ( strcmp(pThread->szBeginDate,"-")==0 )
			objWI.GetWebInterface()->raw_GetUserArticleList( atol((LPCSTR)pThread->strID),L"",A2W(pThread->szEndDate),&retValue);
		else
			objWI.GetWebInterface()->raw_GetUserArticleList( atol((LPCSTR)pThread->strID),A2W(pThread->szBeginDate),A2W(pThread->szEndDate),&retValue);
