...
CSendPacket& CSendPacket::operator << (float data)
{
	if (HasData(sizeof(data)))
	{
		*(float *)m_pCursor = data;
		m_pCursor += sizeof(data);
	}
...
{
	(*this)<<h.flag<<h.from<<h.cmd<<h.seq<<h.sid<<h.dtype<<h.did;
...
CRecvPacket& CRecvPacket::operator >> (float &data)
{
	if (HasData(sizeof(data))) 
	{
		data = *(float *) m_pCursor;
		m_pCursor += sizeof(data);
	}
	else
	{
