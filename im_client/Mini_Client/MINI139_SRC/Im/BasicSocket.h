...
#if !defined(BasicSocketException12345)
#define BasicSocketException12345

typedef const struct sockaddr* LPCSOCKADDR;

class CBasicSocketException : public CException
{
	DECLARE_DYNAMIC(CBasicSocketException)
public:
// Constructor
	CBasicSocketException(char* pchMessage);

public:
	~CBasicSocketException() {}
...
class CBasicSocket : public CObject
{
	DECLARE_DYNAMIC(CBasicSocket)
public:
	SOCKET m_hSocket;
	CBasicSocket() { m_hSocket = NULL; }
	void Cleanup();
	void Create(int nType = SOCK_STREAM);
	void Close();
	SOCKET Bind(LPCSOCKADDR psa);
	void Listen();
	void Connect(LPCSOCKADDR psa);
	BOOL Accept(CBasicSocket& s, LPSOCKADDR psa);
	...
	int ReceiveM(char* pch, const int nSize, const int nSecs,SOCKET s1,SOCKET s2);
	int SendDatagram(const char* pch, const int nSize, LPCSOCKADDR psa, 
		const int nMicroSecs);
