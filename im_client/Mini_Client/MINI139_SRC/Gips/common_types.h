...
typedef int (*pMakeIMPacket)(void *pObj,char *pData,char * pOutData,int &datalen,int port);
namespace webrtc {
...
public:
	...
	virtual int SetTalkType(bool video){_bVideo = video;return 0;}
	virtual int GetTalkType() const{return _bVideo;}
protected:
    virtual ~Transport() {}
    Transport() {}
protected:
...
struct IM_GIPS_HEAD
{
	unsigned short	flag:4;		// gips包标志位 (0~15)
	unsigned short	cmd:12;		// 命令字
	unsigned short	len;		// 数据长度
	unsigned int	did;		// 接收者id
	unsigned int	socketId;	// 会话id
	unsigned int	seq;		// 数据分包序号
};

struct IM_GIPS_LOGIN_REQ:public IM_GIPS_HEAD
{
	unsigned int  rtp;  //rtp or rtcp
	unsigned int  video; //video or audio
