...
// CTabWndSearch 对话框
HANDLE  g_hSmsHandle    = NULL;
DWORD   g_dwSmsThreadId = 0;
IMPLEMENT_DYNAMIC(CTabWndSearch, CDialog)

CTabWndSearch::CTabWndSearch(CWnd* pParent /*=NULL*/)
	: CDialog(CTabWndSearch::IDD, pParent)
{

}

CTabWndSearch::~CTabWndSearch()
{
}

void CTabWndSearch::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CTabWndSearch, CDialog)
	ON_WM_SIZE()
	ON_XT_MESSAGE()
END_MESSAGE_MAP()

BEGIN_XT_MESSAGE_MAP(CTabWndSearch)
	ON_XT_COMMAND(CMD_XTREAM_SEARCH_ACK, OnSearchUser)
END_XT_MESSAGE_MAP()
// CTabWndSearch 消息处理程序
BOOL CTabWndSearch::OnInitDialog()
{
	CDialog::OnInitDialog();

	CRect rect;
	GetClientRect(&rect);
	m_PeopleSearchList.Create(GetSafeHwnd(), _T("DUIPeopleSearchList"), UI_WNDSTYLE_CHILD, 0, rect.left, rect.top, rect.Width(), rect.Height()); 
	m_PeopleSearchList.ShowWindow(true); 
...
void CTabWndSearch::OnSize(UINT nType, int cx, int cy)
{
	...
	CRect rect;
	GetClientRect(&rect);
	if(::IsWindow(m_PeopleSearchList.GetHWND()))
	{		
		::MoveWindow(m_PeopleSearchList.GetHWND(), rect.left, rect.top, rect.Width(), rect.Height(), TRUE);
	}
}

void CTabWndSearch::UpdateSearchList(CLUSTER_INFO * pCluster,CString strItem)
{
	m_PeopleSearchList.SelPage(0);
	m_PeopleSearchList.RemoveAll();
	...
	for(itMapTeamFriend= m_mapTeamFriend.begin();itMapTeamFriend != m_mapTeamFriend.end();itMapTeamFriend++)
	{
		...
		{
			string strOutImg = (LPCTSTR)strGifFile;
			m_PeopleSearchList.AddFriend(strOutImg.c_str(), itMapTeamFriend->second, itMapTeamFriend->first->user_sign, (DWORD)itMapTeamFriend->first, TRUE, 26);
		}
		else
		{
	...
	for(itMapFriend= m_mapFriend.begin();itMapFriend != m_mapFriend.end();itMapFriend++)
	{
		...
		{
			string strOutImg = (LPCTSTR)strGifFile;
			m_PeopleSearchList.AddFriend(strOutImg.c_str(), itMapFriend->second, itMapFriend->first->user_sign, (DWORD)itMapFriend->first, TRUE, 26);
		}
		else
		{
			string strOutImg = CutRoundImg(strFaceFile, 24, 24, FALSE);
	...
	// 没有查找到好友
	if(m_PeopleSearchList.GetCount() == 0)
	{	
		m_PeopleSearchList.SelPage(1);
	}
}

// 查找用户
void CTabWndSearch::OnSearchUser(const char *data, int nRetCode)
{
	if ( nRetCode==0)
	{
		//确保回来的应答是最后一次发的请求对应的应答，如果是之前的请求应答则丢弃
		if( m_PeopleSearchList.m_nLastReqSeq != g_pProtocol->GetLastAckSequenceId() )
		{
			return;
		}
		...
		for ( i=0; i<count; i++ )
		{
			...
			{	// 不能和自己聊天
				CMessageBox::Model(GetSafeHwnd(),"不能和自己聊天！");
	...
	{	// 查找联系人失败
		CMessageBox::Model(GetSafeHwnd(),"未查找到联系人！");
		//m_editName.SetFocus();
	}
...
////根据拼音联想搜索，规则如下:
	...
	//5.支持汉字部分匹配
/////////////////////////////////////////////////////////////////
void CTabWndSearch::ShowSearchList(CString &strKey)
{
...
*/
	...
	{
		m_PeopleSearchList.AddGroup("我的团队");
	}
	map<FRIEND_INFO*,CString>::iterator itMapTeamFriend;
	for(itMapTeamFriend= m_mapTeamFriend.begin();itMapTeamFriend != m_mapTeamFriend.end();itMapTeamFriend++)
	{
		...
		{
			string strOutImg = (LPCTSTR)strGifFile;
			m_PeopleSearchList.AddFriend(strOutImg.c_str(), itMapTeamFriend->second, itMapTeamFriend->first->user_sign, (DWORD)itMapTeamFriend->first);
		}
		else
		{
	...
	{
		m_PeopleSearchList.AddGroup("联系人");
	}
	map<FRIEND_INFO*,CString>::iterator itMapFriend;
	for(itMapFriend= m_mapFriend.begin();itMapFriend != m_mapFriend.end();itMapFriend++)
	{
		...
		{
			string strOutImg = (LPCTSTR)strGifFile;
			m_PeopleSearchList.AddFriend(strOutImg.c_str(), itMapFriend->second, itMapFriend->first->user_sign, (DWORD)itMapFriend->first);
		}
		else
		{
			string strOutImg = CutRoundImg(strFaceFile, 30, 30, FALSE);
	...
	if(!m_mapGroup.empty())
	{
		m_PeopleSearchList.AddGroup("群组");
	}
	map<CLUSTER_INFO*,CString>::iterator itMapGroup;
	for(itMapGroup= m_mapGroup.begin();itMapGroup != m_mapGroup.end();itMapGroup++)
	{
		m_PeopleSearchList.AddFriend("SysBtn\\icon_group.png", itMapGroup->second, "", (DWORD)itMapGroup->first, FALSE);
	}
	...
	{	
		...
		m_PeopleSearchList.SetTempSessionBtnText(strText);
	}
	else
	{	// 默认显示第一项
		m_PeopleSearchList.SetCurSel(1);
	}
}

///////////////////////////////////////////////////////////////////////
////查找群和好友中搜索到的结果
BOOL CTabWndSearch::ShowSearchRes(CString strKey,FRIEND_INFO *pFriend,CLUSTER_INFO *pCluster)
{
	map<int,int> mapGroup;
	BOOL bFlag = FALSE;
	int  nFolder = 0;
...
///////////////////////////////////////////////////////////////////////////////////////////
////匹配规则判断
BOOL CTabWndSearch::IsKeyMatch(CString strKey, FRIEND_INFO *pFriend,CLUSTER_INFO *pCluster)
{
...
BOOL CTabWndSearch::KeySearch(CString strKey,CString strName,FRIEND_INFO *pFriend,CLUSTER_INFO *pCluster)
{
		////判空
	if(strKey.IsEmpty())
	{
	...
	BOOL bName = IsIncludeChinese(strName); ////待查内容是否包含多个汉字
	BOOL bKey  = IsIncludeChinese(strKey);  ////输入关键字是否包含多个汉字
	...
	else if(bName&&!bKey) // 关键字不含,待匹配字符含汉字
	{	
	...
	if(bPinYin)
	{
		...
		////vecStr = HZ2Pinyin(szRes); /////汉字转拼音
 		vecStr  =  GetLetter(strName); /////汉字转拼音
...
BOOL CTabWndSearch::KeySearch(CString strKey,CString strName)
{
		////判空
	if(strKey.IsEmpty())
	{
...
BOOL CTabWndSearch::IsIncludeChinese(const char *str)
{
	char c;
	while(1)
	{
...
vector<CString> CTabWndSearch::GetLetter(CString strText)
{
	TBYTE ucHigh, ucLow;
	int  nCode,j;
	CString strValue="";
	vector<CString> VecPinyin;
	for (int i=0; i<strText.GetLength(); i++)
	{
		...
		for(j=0;j<strRes.GetLength();j++)
		{
			strRes.SetAt(j,(strRes.GetAt(j)+32));
		}
		strValue = strRes;
		VecPinyin.push_back(strValue);
		i++;
	}
	return VecPinyin;
}

CString CTabWndSearch::FindLetter(int nCode)
{
	CString strValue;
	switch(nCode)
	{
...
// 消息路由
BOOL CTabWndSearch::PreTranslateMessage(MSG* pMsg)
{
	BOOL bRlt = CDialog::PreTranslateMessage(pMsg);
	if(::IsWindow(m_PeopleSearchList.GetHWND()))
	{	// 让duilib窗口能够响应到windows消息
		if((pMsg->message == WM_KEYDOWN && (pMsg->wParam == VK_DOWN || pMsg->wParam == VK_UP))
			|| pMsg->message == WM_MOUSEWHEEL)
			m_PeopleSearchList.HandleMessage(pMsg->message, pMsg->wParam, pMsg->lParam);
	}
...
// 打开聊天窗口
void CTabWndSearch::OpenCharWnd()
{
	m_PeopleSearchList.OpenCharWnd();
}

// 是否显示临时会话按钮
void CTabWndSearch::ShowTempSessionBtn(BOOL bShow)
{
	m_PeopleSearchList.ShowTempSessionBtn(bShow);
}

//////////////////////////////////////////////////////////////////////
DuiLib::UILIB_RESOURCETYPE CPeopleSearchList::GetResourceType() const
{
	return DuiLib::UILIB_ZIPRESOURCE;
}

LPCTSTR CPeopleSearchList::GetResourceID() const
{
	return MAKEINTRESOURCE(IDR_ZIPRES1);
}

void CPeopleSearchList::InitWindow() 
{
...
LRESULT CPeopleSearchList::HandleMessage(UINT uMsg,WPARAM wParam,LPARAM lParam)
{
	// 禁止窗口最大化
	if(WM_NCLBUTTONDBLCLK != uMsg)
	{
		if(uMsg == WM_KEYDOWN)
		{
			if ((wParam == VK_DOWN || wParam == VK_UP) && m_PaintManager.GetFocus() == NULL)	
			{
				m_PaintManager.SetFocus(m_pSearchList);
			}
		}
...
DuiLib::CControlUI* CPeopleSearchList::CreateControl(LPCTSTR pstrClass)
{
	if (_tcsicmp(pstrClass, _T("SearchList")) == 0)
	{
...
void CPeopleSearchList::AddFriend(LPCTSTR szImgPath, LPCTSTR szText, LPCTSTR szRemarks, DWORD tag, BOOL bUser /*= TRUE*/, int nHeight /*= 32*/)
{
	m_pSearchList->AddFriend(szImgPath, szText, szRemarks, tag, bUser, nHeight);
}

void CPeopleSearchList::AddGroup(LPCTSTR szText, int nHeight /*= 32*/)
{
	m_pSearchList->AddGroup(szText, nHeight);
}

void CPeopleSearchList::RemoveAll()
{
...
int CPeopleSearchList::GetCount()
{
	return m_pSearchList->GetCount();
}

void CPeopleSearchList::SetCurSel(int nIndex)
{
	m_pSearchList->SelectItem(nIndex);
}

void CPeopleSearchList::Notify(DuiLib::TNotifyUI& msg)
{
	DuiLib::WindowImplBase::Notify(msg);
	
	if (_tcsicmp(msg.sType, DUI_MSGTYPE_ITEMSELECT) == 0)
	{
		DuiLib::CControlUI *pItem = m_pSearchList->GetItemAt(msg.wParam);
		if(pItem != NULL && pItem->GetTag() == -1)
		{
			if(msg.lParam > msg.wParam)
			{	// 按的向上键
				pItem = m_pSearchList->GetItemAt(msg.wParam-1);
				if(pItem != NULL && pItem->GetTag() != -1)
				{
					msg.wParam -= 1;
				}
	...
	{
		if(msg.pSender == m_pBtnTempSession)
		{
...
void CPeopleSearchList::SelPage(int nIndex)
{
	m_pTab->SelectItem(nIndex);
}

void CPeopleSearchList::SetTempSessionBtnText(LPCTSTR pText)
{
	m_pBtnTempSession->SetText(pText);
}

// 根据选择项，打开聊天窗口
void CPeopleSearchList::OpenCharWnd()
{
	// 获取转接对象
	...
	{	// 打开好友聊天窗口
		...
		{
			CMessageBox::Model(NULL,"不能和自己聊天");
			return;
		}
		/*CP2PSendMsgDlg*/CDuiP2PSendMsgDlg *pDlg;
		pDlg = g_pMessageHandleWnd->GetP2PSendMsgDlg(pFriend);
...
// 是否显示临时会话按钮
void CPeopleSearchList::ShowTempSessionBtn(BOOL bShow)
{
	m_pBtnTempSession->SetVisible(bShow);
}

///////////////////////////////////////////////////////////////////////////////////////////////////////////
CSearchListUI::CSearchListUI(DuiLib::CPaintManagerUI& paint_manager)
	: paint_manager_(paint_manager)
{
}

CSearchListUI::~CSearchListUI()
{
}

bool CSearchListUI::AddGroup(LPCTSTR szText, int nHeight /*= 32*/)
{
	DuiLib::CListContainerElementUI* pListElement = NULL;
	DuiLib::CDialogBuilder dlgBuilder;
	pListElement = static_cast<DuiLib::CListContainerElementUI*>(dlgBuilder.Create(_T("SearchListGroupItem.xml"), (UINT)0, NULL, &paint_manager_));
	if (pListElement == NULL)
		return false;

	DuiLib::CButtonUI *pBtn = static_cast<DuiLib::CButtonUI*>(pListElement->FindSubControl("groupnameBtn"));
	if(pBtn == NULL)
		return false;

	DuiLib::CDuiRect rcTextPading(10, 0, 0, 0);
	pBtn->SetTextPadding(rcTextPading);
	pBtn->SetText(szText);
	pBtn->SetTag(-1);				// tag为-1表示item为分组

	pListElement->SetTag(-1);		// tag为-1表示item为分组
	pListElement->SetFixedHeight(nHeight);

	if(strcmp(szText, "") == 0)
	{
		pListElement->SetEnabled(false);
	}

	return CListUI::Add(pListElement);
}

bool CSearchListUI::AddFriend(LPCTSTR szImgPath, LPCTSTR szText, LPCTSTR szRemarks, DWORD tag, BOOL bUser /*= TRUE*/, int nHeight /*= 32*/)
{
	DuiLib::CListContainerElementUI* pListElement = NULL;
	DuiLib::CDialogBuilder dlgBuilder;
	pListElement = static_cast<DuiLib::CListContainerElementUI*>(dlgBuilder.Create(_T("SearchListItem.xml"), (UINT)0, NULL, &paint_manager_));
	if (pListElement == NULL)
		return false;

	DuiLib::CButtonGifUI *pHeadPic = static_cast<DuiLib::CButtonGifUI*>(pListElement->FindSubControl("HeadPic"));
	DuiLib::CLabelUI *pLabelName = static_cast<DuiLib::CLabelUI*>(pListElement->FindSubControl("nickname"));
	DuiLib::CLabelUI *pLabelRemarks = static_cast<DuiLib::CLabelUI*>(pListElement->FindSubControl("note"));
	if(pLabelName == NULL || pLabelRemarks == NULL)
		return false;
	...
	{
        pHeadPic->SetBkImage("");
		pHeadPic->SetNormalGifFile(strPicFile);
	}
	else
	{
		CString strFile;
		strFile.Format("file='%s' dest='1,1,%d,%d'", szImgPath, 1+(nHeight-2), 1+(nHeight-2));
		pHeadPic->SetBkImage(strFile);
		pHeadPic->SetNormalGifFile("");
	}
	/*pLabelName->SetBkImage(strFile);*/
	pLabelName->SetTag((DWORD)bUser);		// 标识是否为好友

	DuiLib::CDuiRect rcTextPading(10, 0, 0, 0);
	pLabelName->SetTextPadding(rcTextPading);
	pLabelName->SetText(szText);
	

	pLabelRemarks->SetText(szRemarks);
	...
	{
		pLabelName->SetFixedWidth(pLabelName->CalcRealWidth()+10);
		return true;
	}
