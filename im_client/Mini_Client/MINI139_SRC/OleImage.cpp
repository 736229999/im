...
HRESULT COleImage::LoadFromFile(BSTR lpszPathName, CRichEditCtrl *pContainer, UINT nMaxWidth)
{
	...
	if (m_nFrmCount > 1) { // 总帧数超过1时
		...
		for (int i = 0; i < m_nFrmCount; i++) {
			...
			pDelays[i] = (((pDelays[i] < 10 ? 10 : pDelays[i]) * 10) + (MIN_FRM_DELAY / 2)) / MIN_FRM_DELAY;
...
HRESULT COleImage::LoadFromFile(BSTR lpszPathName, DuiLib::CRichEditUI *pContainer, UINT nMaxWidth)
{
	// 先清除以前加载过的图像
	FinalRelease();

	// 保存容器窗口指针，便于更换帧时调用容器窗口的InvalidateRect函数
	m_pContainerDui = pContainer;
...
{
	// 必须要帧数大于1，且容器窗口指针存在时
	if (m_nFrmCount > 1 && (m_pContainer != NULL || m_pContainerDui != NULL)) {
		// 增加当前时间
		++m_iCurTime;
		// 超过总的延迟时间时，又从0开始
		...
		if (m_pFrmIndexes[m_iCurTime] != m_iPrevFrm) {
			if(m_pContainer != NULL)
				...
				m_pContainerDui->GetManager()->Invalidate(m_rcPos);
			// 更新上一次绘制的帧索引为当前的帧索引
