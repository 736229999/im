...
int CBusinessExGSrv::SendReply()
{
	...
	//加密数据
	long nDataLength = pSendData->data_len;
	...
	CLog::Log(typeid(*this).name(),CLog::TYPE_DEBUG,"返回命令 (0x%.4x) %d字节",
				pSendHead->cmd,pSendData->data_len);
...
void CBusinessExGSrv::SetRowPacket(BYTE* pRowPacket,int nPacketLength)
{
	...
	long	  nProtocolDataLen = pRecData->data_len - sizeof(XT_HEAD);
