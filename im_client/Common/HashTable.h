...
public:
	...
	{
		m_hashElem = HASHELEM;
		m_tableLock = new CRITICAL_SECTION[m_hashElem];
		m_hash = new tagLink * [m_hashElem];
		for(int i = 0; i < m_hashElem; ++i)
			InitializeCriticalSection(&m_tableLock[i]);
		m_nElemCount = 0;
	...
	{
		for(int i = 0; i < m_hashElem; ++i)
			DeleteCriticalSection(&m_tableLock[i]);
		tagLink * pNode, * pDelNode;
		for(i = 0; i < m_hashElem; ++i)
		{
			pNode = m_hash[i];
			while(pNode)
			{
	...
	T * AddHash(uint32 id)
	{
		CMyLock MyLock(&m_tableLock[id % m_hashElem]);
