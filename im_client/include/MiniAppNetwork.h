...
#define MINIAPPNETWORK_API __declspec(dllexport)
#else
#define MINIAPPNETWORK_API __declspec(dllimport)
#endif
...
public:
	...
	*/ 
	int UserLogin(uint32 uid,uint32 uip,uint16 uport,const char * pSessionKey = NULL,const char * pP2PSessionKey = NULL,uint32 lanip = 0,uint16 lanport = 0);
	...
	*/ 
	...
	返 回 值：0 成功，< 0 失败
	*/ 
	int SendDataTo(const char * data,int size,int retrys = 0,uint16 interval = 0,uint32 ip = 0,uint16 port = 0);
