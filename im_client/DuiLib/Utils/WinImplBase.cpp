...
//////////////////////////////////////////////////////////////////////////
std::tr1::shared_ptr<BYTE> WindowImplBase::m_lpResourceZIPBuffer=NULL;
	...
	case UILIB_ZIPRESOURCE:
		{
			if(m_lpResourceZIPBuffer == NULL)
			{
				...
				if( dwSize == 0 )
					return 0L;
				m_lpResourceZIPBuffer = std::tr1::shared_ptr<BYTE>(new BYTE[ dwSize ]);
				if (m_lpResourceZIPBuffer != NULL)
				{
					::CopyMemory(m_lpResourceZIPBuffer.get(), (LPBYTE)::LockResource(hGlobal), dwSize);
				}
#if defined(WIN32) && !defined(UNDER_CE)
				::FreeResource(hResource);
#endif
				m_PaintManager.SetResourceZip(m_lpResourceZIPBuffer.get(), dwSize);
			}
		}
		break;
	}
