...
{
	if(hDC == NULL || hRgn == NULL || hOldRgn == NULL)
	{
		ASSERT(FALSE);
		return;
	}
...
void CRenderEngine::DrawLine(HDC hDC, int x1, int y1, int x2, int y2, int nSize, DWORD dwPenColor,int nStyle /*= PS_SOLID*/)
{
	ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
...
void CRenderEngine::DrawText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, int iFont, UINT uStyle)
{
	...
	if ( pManager->IsBackgroundTransparent() || pManager->IsUseGdiplusText())
	{
		...
		if ((uStyle & DT_CALCRECT) != 0)
		{
			...
			WCHAR buf[1024];
			CStringW str = CT2CW(pstrText); // 将T字符转换成宽字符
			wcscpy(buf, str); 

			graphics.MeasureString(buf, -1, &font, rectF, &stringFormat, &bounds);
		...
		else
		{
			...
			CStringW str = CT2CW(pstrText); // 将T字符转换成宽字符
			wcscpy(buf, str); 
			graphics.DrawString(buf, -1, &font, rectF, &stringFormat, &brush);
		}
