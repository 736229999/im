...
namespace DuiLib
{
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		else if( uMsg == WM_KEYDOWN && (TCHAR(wParam) == VK_UP || TCHAR(wParam) == VK_DOWN) ){
	...
	LRESULT CEditWnd::OnEditChanged(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		if(m_pOwner->m_uMaxChar > 0)
		{
			long nStartChar = 0, nEndChar = 0;
			int maxSize = m_pOwner->m_uMaxChar;
			CDuiString strName,allName;
			CDuiString tmp;
			strName = pstr;
			if(strName == m_pOwner->m_strOld)
				return 0;
			int iLen =0;//ÎÄ×Ö¸öÊý
			...
			int iLenOld=0;
			for (int i=0;m_pOwner->m_strOld[i];i++) {		
				if (m_pOwner->m_strOld[i] < 0)		   
					i++;   			
				iLenOld++;
			}	
	...
	CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), 
		...
		m_dwTipValueColor(0xFFBAC0C5), m_strOld(""), m_nStartCharOld(0), m_nEndCharOld(0)
	{
	...
	{
		if( m_pWindow != NULL ) SNDMSG(*m_pWindow, EM_GETSEL, (WPARAM)&nStartChar, (LPARAM)&nEndChar);
	}	
