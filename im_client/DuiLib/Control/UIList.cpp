...
CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1)
{
	...
	m_ListInfo.bDrawHotBkColor = true;
...
void CListUI::SetItemIsDrawHotBkColor(bool bDraw)
{
	m_ListInfo.bDrawHotBkColor = bDraw;
	Invalidate();
}
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("itemdrawhotbkcolor")) == 0 ) SetItemIsDrawHotBkColor(_tcscmp(pstrValue, _T("true")) == 0);
...
void CListElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
{
	...
	if( (m_uButtonState & UISTATE_HOT) != 0 && pInfo->bDrawHotBkColor ) {
...
void CListContainerElementUI::DoEvent(TEventUI& event)
{
	...
	if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_RBUTTONDOWN )
	{
		if( IsEnabled() ){
			...
			{
				m_pManager->SendNotify(this, DUI_MSGTYPE_RBUTTONDOWN, event.wParam, event.lParam, true);
			}
		}
		return;
	}
    ...
    if( event.Type == UIEVENT_MOUSEENTER )
    {
		...
		if(m_vecBtnUI.size() > 0)
		{
			for(int nBtn = 0; nBtn < (int)m_vecBtnUI.size(); nBtn++)
			{
				m_vecBtnUI[nBtn]->SetVisible(true);
			}
		}
        return;
    }
    if( event.Type == UIEVENT_MOUSELEAVE )
    {
		...
		if(m_vecBtnUI.size() > 0)
		{
			for(int nBtn = 0; nBtn < (int)m_vecBtnUI.size(); nBtn++)
			{
				m_vecBtnUI[nBtn]->SetVisible(false);
			}
		}
        return;
    }
...
void CListContainerElementUI::SetDrawBkColor(DWORD dwBackColor)
{
	m_dwDrawBackColor = dwBackColor;
}

bool CListContainerElementUI::IsItemHot()
{
	if( (m_uButtonState & UISTATE_HOT) != 0 )
		return true;
	return false;
...
void CListContainerElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
{
    ...
    if( !pInfo->bAlternateBk || m_iIndex % 2 == 1 ) iBackColor = pInfo->dwBkColor;
  	...
  	if(m_dwDrawBackColor != NULL)
  		iBackColor = m_dwDrawBackColor;
...
void CListContainerElementUI::SetMousemoveBtnVisible(std::vector<CButtonUI*>& vecBtnUI)
{
	m_vecBtnUI = vecBtnUI;
}
