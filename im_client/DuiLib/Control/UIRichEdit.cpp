...
err:
	...
	CRichEditUI::CRichEditUI() : m_pTwh(NULL), m_bVScrollBarFixing(false), m_bWantTab(true), m_bWantReturn(true), 
		...
		m_iLimitText(/*cInitTextMax*/-1), m_lTwhStyle(ES_MULTILINE), m_bInited(false), m_chLeadByte(0),m_uButtonState(0),
	...
	void CRichEditUI::SetLinkMask()
	{
		DWORD mask = GetEventMask();
		SetEventMask(mask | ENM_LINK | ENM_MOUSEEVENTS | ENM_SCROLLEVENTS | ENM_KEYEVENTS);
