...
namespace DuiLib
{
	...
	int CLabelUI::CalcRealWidth()
	{
		RECT rcText = {0};

		CRenderEngine::DrawText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText, m_dwTextColor, m_iFont, DT_CALCRECT | m_uTextStyle);
		return rcText.right - rcText.left + m_rcTextPadding.left + m_rcTextPadding.right;
	}
	...
	void CLabelUI::DoEvent(TEventUI& event)
	{
		...
		{
			m_pManager->SendNotify(this, DUI_MSTTYPE_MOUSEHOVER, 0, 0, true);
		}
