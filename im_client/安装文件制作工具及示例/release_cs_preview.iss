...
AppVersion=2.4.0.0
AppPublisher=飞牛网
DefaultDirName={pf}\MouMou_cs
DefaultGroupName=MouMou_cs
UninstallDisplayName= 飞牛客服工作台 2.4.0.0
UninstallDisplayIcon={app}\MouMou.exe
Compression=lzma2
SolidCompression=yes
SetupIconFile="ReleaseVersionSeller\MouMou.ico"
OutputDir="Release"
OutputBaseFilename=MouMou_cs_preview_2.4.0.0
...
[Icons]
Name: "{group}\飞牛客服工作台"; Filename: "{app}\MouMou.exe";WorkingDir: "{app}"
Name: "{group}\{cm:UninstallProgram,飞牛客服工作台}"; Filename: "{uninstallexe}"
Name: "{userdesktop}\飞牛客服工作台"; Filename: "{app}\MouMou.exe"
...
Root: HKCR; Subkey: "feiniu-cs"; ValueType: string; ValueName: "URL Protocol" ; ValueData: "{app}\MouMou.exe"
Root: HKCR; Subkey: "feiniu-cs"; ValueType: string; ValueData: "FeiniuProtocol"
Root: HKCR; Subkey: "feiniu-cs\DefaultIcon";ValueType: string; ValueData: "{app}\MouMou.exe,1"
Root: HKCR; Subkey: "feiniu-cs\shell\open\command"; ValueType: string;ValueData: "{app}\MouMou.exe %1"

Root: HKCR; Subkey: "feiniu-cs";Flags: uninsdeletekey 
Root: HKCR; Subkey: "feiniu-cs\DefaultIcon";Flags: uninsdeletekey 
Root: HKCR; Subkey: "feiniu-cs\shell\open\command";Flags: uninsdeletekey 
...
var
  ...
  ResultCode: Integer;
begin
    HasRun := FindWindowByWindowName('飞牛客服工作台');
    while HasRun<>0 do
    begin
      Res:= MsgBox('检测到飞牛客服工作台正在运行，请确认退出', mbInformation, MB_OK );
      if  Res <> 7 then 
      begin
...
begin
    ...
    If RegQueryStringValue(HKEY_LOCAL_MACHINE, 'SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\飞牛客服工作台_is1', 'UninstallString', uicmd) then
    begin
...
[run]
Filename: "{app}\MouMou.exe";Description:"立即体验最新飞牛客服工作台"; StatusMsg:"别急，等一下就好"; Flags: nowait postinstall skipifsilent
