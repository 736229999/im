...
void CListBoxST::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
	...
	if (m_pImageList)
		lpMeasureItemStruct->itemHeight = max(nHeight, m_szImage.cy + CY_BORDER*2)*3/2;
	else
		lpMeasureItemStruct->itemHeight  = nHeight*3/2;

	lpMeasureItemStruct->itemHeight += CY_BORDER*3;
} // End of MeasureItem
...
void CListBoxST::DrawItem(LPDRAWITEMSTRUCT pDIStruct)
{
	...
	if (m_pImageList)
	{
		//rcIcon.left=(rcItem.Width()-rcIcon.Width())/2;  //wangtao	
		rcIcon.left=50;
		rcIcon.right = rcIcon.left + m_szImage.cx + CX_BORDER*2;
		//rcIcon.right=(rcItem.Width()+rcIcon.Width())/2;  //wangtao
	...
	// Calculate rcText
	rcText.left = 30;
	rcText.top=rcIcon.bottom+2;   //wangtao
...
DWORD CListBoxST::OnDrawIconBackground(int nIndex, CDC* pDC, CRect* prcItem, CRect* prcIcon, BOOL bIsDisabled, BOOL bIsSelected, COLORREF crSuggestedColor)
{
  ...
  pDC->FillSolidRect(0, prcIcon->top, prcItem->Width(), prcItem->Height(), crSuggestedColor); //wangtao
	return 0;
