...
/** 示例.
 @code
	...
	void test()
	{
		...
		out << ss << name << v8 << v16 << v32 << v << vf << vd << vl << vul; // 导出固定长度类型的数据到缓冲区
		out.push(mydata, sizeof(mydata)); // 导出一个数组（缓冲区）到该缓冲区
		if (!out)
		{
			Error("error\n");
		}
		...
		in.skip<uint32>();	// 跳过一个uint32，我们不关心
		in >> v >> vf >> vd >> vl >> vul;
...
//使用GCC编译时,该段代码移植buffer.cpp文件中分离编译
#if FCL_COMPILER == FCL_COMPILER_MSVC
