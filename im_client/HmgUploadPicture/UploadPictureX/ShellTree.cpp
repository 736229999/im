...
void CShellTree::PopulateTree() 
{
    ...
    if (SUCCEEDED(hr))
    {
		...
		tvi.pszText    = _T("×ÀÃæ");
		tvi.cchTextMax = lstrlen(_T("×ÀÃæ"));
	//

		// Add the item to the tree
		HTREEITEM hItem = InsertItem(_T("×ÀÃæ"));

       // Fill in the tree view from the root.
       FillTreeView(lpsf, NULL, hItem);
        ...
        if (SUCCEEDED(hr))
        {
            ...
            while (S_OK==lpe->Next(1, &lpi, &ulFetched))
            {
                ...
                if (ulAttrs & (SFGAO_HASSUBFOLDER | SFGAO_FOLDER))
                {
                   ...
                   lpifqThisItem=0;
                } // end if (ulAttrs & (SFGAO_HASSUBFOLDER | SFGAO_FOLDER))

                lpMalloc->Free(lpi);  //Free the pidl that the shell gave us.
                lpi=0;
            } // end while (S_OK==lpe->Next(1, &lpi, &ulFetched))
        } // end if (SUCCEEDED(hr))
    }
    else
	{
...
bool CShellTree::SearchTree(HTREEITEM treeNode,	CString szSearchName,	FindAttribs attr)
{
			...
			if (SUCCEEDED(hr))
			{
				...
				if (ulAttrs & (SFGAO_FILESYSTEM))
				{
					if(SHGetPathFromIDList(lptvid->lpifq,szCompare.GetBuffer(MAX_PATH)))
					{
						...
						szCompare.MakeUpper();
				//		write_log_info("CShellTree::SearchTree", "SHGetPathFromIDList attr = %d, szCompare = %s.", (int) attr, szCompare);
				...
				} // end if (ulAttrs & (SFGAO_FILESYSTEM))
	...
	} // end while(treeNode && bRet==false)

	return bRet;
}

bool CShellTree::GotoMyComputer()
{
#if 0
	HTREEITEM	_hItem = GetPrevSiblingItem(GetSelectedItem());
	SelectItem(_hItem);
	...
	while(_hItem && bRet==false)
	{
		lptvid=(LPTVITEMDATA)GetItemData(_hItem);
		if (lptvid && lptvid->lpsfParent && lptvid->lpi)
		{
			hr=lptvid->lpsfParent->BindToObject(lptvid->lpi,0,IID_IShellFolder,(LPVOID *)&lpsf2);
			if (SUCCEEDED(hr))
			{
				...
				if (ulAttrs & (SFGAO_FILESYSTEM))
				{
					...
					//		szCompare = drive;
						...
						szCompare.MakeUpper();
						write_log_info("CShellTree::GotoMyComputer", "SHGetPathFromIDList attr = %d, szCompare = %s.", (int) attr, szCompare);

						if(szCompare == szSearchName)
						{
							EnsureVisible(_hItem);
							SelectItem(_hItem);
		...
		_hItem = GetNextSiblingItem(_hItem);
	} // end while(treeNode && bRet==false)
	return bRet;
#endif 
	return false;
...
*
****************************************************************************/
bool CShellTree::TunnelTree(CString szFindPath)
{
	HTREEITEM subNode = GetSelectedItem(); //  GetRootItem();
	...
	if(subNode)
	{
		bool bsearch_tree = SearchTree(subNode, szPathHop, CShellTree::type_drive);
		if(bsearch_tree)
		{
			...
			while(p)
			{
				...
				bsearch_tree = SearchTree(subNode, p, CShellTree::type_folder);
				if(bsearch_tree)
				{
					p = strtok(NULL, delimiter);
					bsearch_succeed = true;
				}
				else
				{
					p = NULL;	
					bsearch_succeed = false;
				}	

			//	if(p != NULL && strlen(p) > 0)	bsearch_succeed = false;	
			} // end while
		}
		else
		{
	...
	return  bsearch_succeed;
}


void CShellTree::TunnelTree2(const CString & szFindPath)
{
	HTREEITEM _hItem = NULL;

	CFileName checkPath(szFindPath);
	CString str = checkPath.GetDescription();
	if(!checkPath.Exist())
	{
		return ;
	}

	bool bsearch_succeed = false;
	_hItem = GetRootItem();
	_hItem = GetChildItem(_hItem);
	while(_hItem != NULL && bsearch_succeed == false)
	{
		Select(_hItem, TVGN_CARET);
		bsearch_succeed = TunnelTree(szFindPath);
		_hItem =  GetNextSiblingItem(_hItem);
	}
}