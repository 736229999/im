...
CNTService::CNTService(const char* szServiceName, const char* szServiceDescprition)
{
	...
	strncpy_s(m_szServiceDesc, szServiceDescprition, sizeof(m_szServiceDesc)-1);
	m_iMajorVersion = 1;
	m_iMinorVersion = 0;
	m_hEventSource = NULL;
...
BOOL CNTService::ParseStandardArgs(int argc, char* argv[])
{
	...
	else if(_stricmp(argv[1], "-h") == 0 || _stricmp(argv[1], "-?") == 0 || _stricmp(argv[1], "/?") == 0)
	{
		printf("-v Spit out version info\n-i install the service\n-u uninstall the service");
		return TRUE;
	}
...
BOOL CNTService::Install()
{
	...
	SC_HANDLE hService = ::CreateService(hSCM,
		...
		NULL);
	if (!hService) {
		printf("CreateService false %d file: %s\n",::GetLastError(),szFilePath);
		::CloseServiceHandle(hSCM);
		return FALSE;
	}
	//   Set   service   description   
	SERVICE_DESCRIPTION   ServiceDesc;   
	ServiceDesc.lpDescription   =   m_szServiceDesc;
