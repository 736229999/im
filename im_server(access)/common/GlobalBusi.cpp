...
int CGlobalBusi::setExpireUserinfo(uint32 userid)
{
	if(userid == 0)
	{
		return RESULT_FAILED;
	}
	m_cMemCBusi.setExpireUserinfo(userid);
	return RESULT_SUCCESS;
}
...
%s %u",		 
	...
	{		
		c_memc->expire(str_3,buyer_Timeout);
	}
	MemcachedUnLock(MEMCACHED_USER_LOCK);
	return true;
}
...
//	int ret = -1;
	...
	if(c_Memcache->GetLinkState() == true) //和memcached正常连接状态
	{
		...
		"key %s id : %s clientip %s clientport %s sessionkey %s",
			key,retval[0],retval[1],retval[2],retval[3]);
...
bool CMemCBusi::setExpireUserinfo(uint32 userid)
{
	char str_1[64] = {0};	
	char str_2[64] = {0};
...
//	int ret = -1;
	...
	if(c_Memcache->GetLinkState() == true) //和memcached正常连接状态
	{
		...
		strncpy(userInfo.username,retval[2],MAX_USERNAME_LEN+1);		
