										 ...
										 ReloginUser* pReloginUserList,uint8& nReloginCount)
{
		...
		action == GSRV_ADDMODIFY_FULLUSERINFO_QUERY::ACTION_MODIFY  ) 
	{
		for(int i = 0 ; i < nUserCount; ++i)
		{
			...
			CLog::Log("COnlineManager::AddModifyOnlineUser", 
				CLog::TYPE_IMPORTANT, "登录注册用户%u 成功 OUT",nClientID);
		}
...
// 			}
			...
			--m_nOnlineNum;
			m_CriticOnlineList.Unlock();
			CLog::Log("DelOnlineUser",CLog::TYPE_IMPORTANT,"DELETE user %u ",pQueryHead->userid[i]);

		}
		else
		{
			CLog::Log("DelOnlineUser",CLog::TYPE_IMPORTANT,"DELETE user %u but NOT FOUND",pQueryHead->userid[i]);
		}
...
bool COnlineManager::GetTUserInfo(CRecvPacket& RecvPacket,CSendPacket& SendPacket)
{
	...
	if(pUser == NULL)
	{
		...
		CLog::Log("COnlineManager:GetTUserInfo",CLog::TYPE_IMPORTANT,"id not found:%u", id);
	}
	else
	{
...
bool COnlineManager::GetUserFullInfo(CRecvPacket& RecvPacket,CSendPacket& SendPacket)
{
	...
	for(i = 0 ; i < pQueryHead->count; ++i)
	{
		...
		if( NULL == pUser)
		{
			...
			CLog::Log("COnlineManager:GetUserFullInfo",CLog::TYPE_IMPORTANT,"id not found:%u", nClientID);
		}
		else
		{
			SendPacket << nClientID 
