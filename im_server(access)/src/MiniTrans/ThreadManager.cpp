...
void CThreadManager::StartThread()
{
	...
	//线程B
	for( i = 0 ; i < CAppManager::GetInstance().GetConfig().GetTMaxTdGrp() ; ++i )
	{
...
void*  CThreadManager::BusinessProcA( LPVOID lpParam )
{
	...
	CBusinessTransferNotify		busTransferNotify(&router);

	CLog::Log("CThreadManager",CLog::TYPE_IMPORTANT,"线程业务注册完成");
