...
bool g_cs_ismonitored(uint32 cs_id, bool &monitored, uint32 &cs_master_id, CGlobalBusi *glob_busi)
{
	...
	if(clientService != NULL)
	{
		monitored = clientService->IsMonitored();
		if(monitored)
		{	
			cs_master_id = clientService->GetDirectorID();
		}
		return true;
	}
...
bool g_cs_get_chatting_buyer(uint32 cs_id, uint32 &chatting_buyer_id, CGlobalBusi *glob_busi)
{
	...
	if(clientService != NULL)
	{
		chatting_buyer_id = clientService->GetChattingBuyerID();
		return true;
	}
...
bool g_cs_set_director(uint32 cs_id, uint32 cs_master_id, CRouterBusiness *glob_busi)
{
	...
	if(clientService != NULL)
	{
		...
		glob_busi->GetGlobalBusi()->UpdateCsMonitor(clientService);
		//CMemcacheUpdateConsultCSInfo(glob_busi, clientService);
	}
	
	// 广播其他服务器，修改上下文
	if(clientService == NULL)
	{
		CsCustOper(glob_busi, UO_SET_MASTER, cs_id, cs_master_id);
	}
...
bool g_cs_set_chatting_buyer(uint32 cs_id, uint32 chattring_buyer_id, CRouterBusiness *glob_busi)
{
	...
	if(clientService == NULL)
	{
		CsCustOper(glob_busi, UO_SET_CHATTING_CUST, cs_id, chattring_buyer_id);
	}
...
*/	
	...
	uint8 count=1;
	if(glob_busi->QueryCSInfo(&cs_id,count,&csinfo_reply) != true)
	{	// 全局服务器中也没有
		return false;
	}		

	strcpy(name, csinfo_reply.m_Csname);
...
// 设置客服正在被谁监控(同时修改上下文及memcache)
bool CUser::g_cs_set_director(uint32 cs_id, uint32 cs_master_id, CRouterBusiness *glob_busi)
{
	if(cs_id < cs_Boundary || cs_id >work_Boundary)
	{
		return false;
...
// 设置客服正在联系的买家(同时修改上下文及memcache)
bool CUser::g_cs_set_chatting_buyer(uint32 cs_id, uint32 chattring_buyer_id, CRouterBusiness *glob_busi)
{
	if(cs_id < cs_Boundary || cs_id >work_Boundary)
	{
		return false;
