...
{
	XT_OFFLINEMSG_REQ OfflineMsgReq;
	m_RecvPacket >> OfflineMsgReq.id;
	XT_OFFLINEMSG_REQ* pOfflineMsgReq = &OfflineMsgReq;
...
// 	}
	...
	shared_ptr<CUser> pUser = m_pRouter->GetUserManager()->Find( pOfflineMsgReq->id , TRUE);
	if( pUser == NULL )
	...
	//businessWebmsg.GetOfflineWokuMsg(pUser.get(), strFriend,m_pRouter->GetDataBaseWeb(),m_pRouter->GetDataBaseMini());

	//CBusinessOfflineMobileMsg busMobileMsg;
	//busMobileMsg.GetOfflineMobileMsg(pUser.get(), m_pRouter->GetDataBaseWeb());
	return RESULT_SUCCESS;
}
