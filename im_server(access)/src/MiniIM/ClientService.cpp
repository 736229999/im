...
bool g_cs_exist(uint32 cs_id, CGlobalBusi *glob_busi)
{
	...
	uint8 count = 1;
	if(glob_busi->QueryCSInfo(&cs_id,count,&csinfo_reply) != MEMCACHED_GET_INFO_SUCCESS)
	{	// 全局服务器中也没有
		return false;
	}		
...
bool g_cs_ismonitored(uint32 cs_id, bool &monitored, uint32 &cs_master_id, CGlobalBusi *glob_busi)
{
	...
	monitored = (csinfo_reply.m_nDirectorID != (uint32)-1 && csinfo_reply.m_nDirectorID != 0);
	if(monitored)
	{	// 正在被监控
		cs_master_id = csinfo_reply.m_nDirectorID;
	}
...
bool g_cs_get_chatting_buyer(uint32 cs_id, uint32 &chatting_buyer_id, CGlobalBusi *glob_busi)
{
	...
	chatting_buyer_id = csinfo_reply.m_nChattingBuyerID;
