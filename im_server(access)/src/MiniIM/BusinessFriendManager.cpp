...
void CBusinessCSQueue::WebLogOut(uint32 toid,uint16 type)   //向WEB服务器发送退出请求
{
    // 构造转发消息结构体
    ...
    * ((uint16*)(pVerifyFriendReq + 5)) = type;

    pVerifyFriendReq = (BYTE*)(sendData.data + sizeof(XT_HEAD) + sizeof(XT_SERVER_TRANSMIT) + sizeof(uint8) + sizeof(uint32) + sizeof(uint16));
    memcpy(pVerifyFriendReq,m_RecvPacket.GetData(),m_RecvPacket.GetSize());

    sendData.data_len = sizeof(XT_HEAD) + sizeof(XT_SERVER_TRANSMIT) + sizeof(uint8) + sizeof(uint32) + sizeof(uint16) + m_RecvPacket.GetSize();
    CLog::Log("CBusinessCSQueue::WebLogOut",CLog::TYPE_ERROR,"向WEB发送让用户%u退出请求",toid);
...
#endif
            int LogoutCount=0;
            if(1 == pUser->m_isLogout)  //有退出线程正在退出，锁住用户
            {
                while(LogoutCount < 10)
                {
                    if(0 == pUser->m_isLogout)
                    {
                        break;
                    }
                    LogoutCount++;
...
//  项目调整
    ...
    else
    {
        ...
        uint8 flag = m_pRecvHead->flag;
        shared_ptr<CUser> pUser = m_pRouter->GetUserManager()->Find(UserInfo.id);
        if( pUser != NULL )
        {
            strcpy(pUser->m_szEmail,UserInfo.email);
            ...
            m_pRouter->GetGlobalBusi()->AddModiUserInfo(pUser.get(),tagGSRV_ADDMODIFY_FULLUSERINFO_QUERY::ACTION_MODIFY,flag);
        }
        if(m_pRecvHead->flag != XT_HEAD::FLAG_MOBREQ)
...
int CBusinessGetGroupInfo::DoWork()
{
    ...
    else
    {
        if(m_pRecvHead->flag != XT_HEAD::FLAG_MOBREQ)
        {
			...
			vector<XT_GROUPINFO_GET_ACK::tagGroupInfo> csGroup;	
            if (pGroupGetReq->nType == 2)
            {
            	CUserManager::GetGroupList(pGroupGetReq->id, csGroup);
                uint8 count = pGroupGetReq->group_count+csGroup.size();
                m_SendPacket<<count ;
            }
            else
            {
            ...
            if(m_pRecvHead->flag == XT_HEAD::FLAG_SELLCNTREQ)
            {
                ...
                {
                    //CSGroup.vecG;
                    for(int j = 0; j < (int)csGroup.size(); ++j)
                    {
                        m_SendPacket << csGroup[j].nGroupPid
                                     << csGroup[j].nGroupID
                                     << csGroup[j].szGroupName;
                    }
                }
                else
                {
...
int CBusinessModifyStatus::DoWork()
{
    ...
    CLog::Log("CBusinessFriendModify",CLog::TYPE_DEBUG,"修改用户%u状态 到 %d IN",UserId,pVerifyFriendReq->status);
    if( pUser != NULL )
    {
...
void CBusinessSendMessage::RouterMsgNotify(uint32 to_id)
{
    ...
    if(pP2PMsgSend->type == XT_MSG::IM_MSG_INSERT)
    {
        ...
        bool bExistMaster = g_cs_exist(pP2PMsgSend->Sender, m_pRouter->GetGlobalBusi());        // 主管
        bool bExistCS = g_cs_exist(pP2PMsgSend->Recver, m_pRouter->GetGlobalBusi());            // 客服
        if(bExistMaster && bExistCS)
        {
            ...
            bool success = g_cs_ismonitored(pP2PMsgSend->Recver, monitored, cs_master_id, m_pRouter->GetGlobalBusi());
            if(success && monitored && cs_master_id == pP2PMsgSend->Sender)
            {
                uint32 nBuyerID = (uint32)-1;       // 买家id
                success = g_cs_get_chatting_buyer(pP2PMsgSend->Recver, nBuyerID, m_pRouter->GetGlobalBusi());
                if(success)
                {
                    ...
                    CAppManager::GetInstance().InitHead(
                        lpSendHead,
                        CMD_SERVERMSG_IND,
                        sizeof(XT_SERVER_TRANSMIT) + sizeof(uint32) + sizeof(uint8) + m_RecvPacket.GetSize(),
                        pP2PMsgSend->Recver);
                    ...
                    *((uint8*)pData) = 1;
                    ...
                    memcpy(pData,m_RecvPacket.GetData(),m_RecvPacket.GetSize());
                    sendData.data_len = sizeof(XT_HEAD) + sizeof(XT_SERVER_TRANSMIT) + sizeof(uint32) + sizeof(uint8) + m_RecvPacket.GetSize();
                    ...
                    if(pUser == NULL)
                    {
                        ...
                        //else
                        //{
                        //    buyer_login_flag = userinfo[0].flag;
                        //}
                    }
                    //else
                    //{
                    //    buyer_login_flag = pUser->flag;
                    //}
                    ...
                    sendData2buyer.data_len = sizeof(XT_HEAD) + sizeof(XT_SERVER_TRANSMIT) + m_RecvPacket.GetSize();
       ...
       || pP2PMsgSend->type == XT_MSG::XT_RECOMENT)
    {
        ...
        bool src_is_cs = g_cs_exist(nFromID, m_pRouter->GetGlobalBusi());
        bool dest_is_cs = g_cs_exist(to_id, m_pRouter->GetGlobalBusi());
        ...
        if( (src_is_cs && !dest_is_cs) || (!src_is_cs && dest_is_cs ) )
        {
            ...
            bool monitored = false;
            uint32 cs_master_id = (uint32)-1;
            bool success = g_cs_ismonitored(cs_id, monitored, cs_master_id, m_pRouter->GetGlobalBusi());
...
int CBusinessReloginNotify::DoWork()
{
    ...
    shared_ptr<CUser> pUser = m_pRouter->GetUserManager()->Find(pReq->userid);
    if( pUser != NULL && pReq->userip && pReq->lanip)
    {
        if (pUser->m_nIP != pReq->userip || (pUser->m_nIP == pReq->userip && pUser->m_nLanIP != pReq->lanip))
            CNotifyProtocol::Relogin(m_pRouter->GetGlobalBusi(),pReq->userid);
    }
...
int CCSetChatWithBuyer::DoWork()
{
    ...
    // 根据id查找客服
    bool is_cs = g_cs_exist(req.idSeller, m_pRouter->GetGlobalBusi());
    if(!is_cs)
    {
        // 未查找到客服
    ...
    // 得到当前正在聊天的买家
    uint32 chatting_buyer_id = (uint32)-1;
    bool success = g_cs_get_chatting_buyer(req.idSeller,chatting_buyer_id,m_pRouter->GetGlobalBusi());
    ...
    // 设置客服正在聊天的买家
    g_cs_set_chatting_buyer(req.idSeller,req.idBuyer,m_pRouter);

    // 判断客服是否正在被监管
    CLog::Log("CCSerChatWithBuyer", CLog::TYPE_DEBUG, "check monitor");
    bool monitored = false;
    uint32 cs_master_id = (uint32)-1;
    success = g_cs_ismonitored(req.idSeller,monitored,cs_master_id,m_pRouter->GetGlobalBusi());
    if(success && monitored)
    {
        // 被监控,发送系统中转消息
...
int CDirectorMonitorStart::DoWork()
{
    ...
    // 根据id查找客服
    bool exist_master = g_cs_exist(req.idMaster,m_pRouter->GetGlobalBusi());
    bool exist_cs = g_cs_exist(req.idSeller,m_pRouter->GetGlobalBusi());
    ...
    g_cs_get_chatting_buyer(req.idSeller,nChattingBuyerID,m_pRouter->GetGlobalBusi());
    ...
    bool monitored = false;
    uint32 cs_master_id;
    g_cs_ismonitored(req.idSeller, monitored, cs_master_id, m_pRouter->GetGlobalBusi());
    ...
    // 开始监控
    g_cs_set_director(req.idSeller,req.idMaster,m_pRouter);
...
int CDirectorMonitorStop::DoWork()
{
    ...
    if(exist_cs)
    {
        g_cs_set_director(req.idSeller,(uint32)-1,m_pRouter);
    }
...
int CMonitorInsertChat::DoWork()
{
    ...
    bool monitored = false;
    uint32 cs_master_id = (uint32)-1;
    bool success = g_cs_ismonitored(req.idSeller,monitored,cs_master_id,m_pRouter->GetGlobalBusi());
    if(!success || !monitored)
    {
    ...
    g_cs_get_chatting_buyer(req.idSeller,buyerID,m_pRouter->GetGlobalBusi());
    // 本地查找买家
    CUserManager *pUserManager = m_pRouter->GetUserManager();
    ...
    *((uint8*)pData) = 0;
    ...
    char csname[256] = {0};
    g_cs_get_name(cs_master_id,csname,m_pRouter->GetGlobalBusi());
...
//  vector<MerchantandSubAccount> merchantInfo;
    ...
    //get all sub users id
    CLog::Log("WebLastMsgReq", CLog::TYPE_IMPORTANT, "WebMoreMsg.did:%d, size:%d ",WebLastMsg.id,ids.size());
...
int CBusinessUpdateCsReq::DoWork()
{
	...
	CUserManager& csManager = CAppManager::GetInstance().GetUserManager();
	CUser* clientService = NULL;
    ...
    if(clientService != NULL)
    {
		if(req.updateoper() == UO_SET_MASTER)
        {
			...
			m_pRouter->GetGlobalBusi()->UpdateCsMonitor(clientService);
			//CMemcacheUpdateConsultCSInfo(m_pRouter, clientService);
        }
