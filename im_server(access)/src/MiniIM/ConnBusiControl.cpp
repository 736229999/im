...
void  BusiControlMyTimeProcNew(union sigval v)
{
	//CLog::Log(__FUNCTION__,CLog::TYPE_DEBUG, "timer:5");
	BusiControlMyTimeProc(v.sival_ptr);
}
...
void CConnBusiControl::HandleTimer()
{
	...
	m_timerStatusReq += m_nTimerDelay;
	if(m_timerStatusReq % (m_nTimerDelay*2) == 0)
	{
		m_timerStatusReq = 0;
...
void CConnBusiControl::TimerBusiRequest()
{
	...
	reqStatusReport.linkCount = /*CAppManager::GetInstance().GetUserManager()->GetElemCount()*/1; 
...
int CConnBusiControl::StartWork(const char * addr,uint16 port)
{
	...
	if(!timeSetEventNew(m_timer,  m_nTimerDelay, m_nTimerResolution, BusiControlMyTimeProcNew, (void*)this ))
	{
		CLog::Log("CConnBusiControl",CLog::TYPE_ERROR,"timer start failed");
	}
