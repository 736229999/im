...
int CBusinessWebMsg::DoWork()
{
	...
	shared_ptr<CUser> pUser = pUserManager->Find(m_pRecvHead->did, TRUE);
	...
	if(pMsg.type == ST_WOKU_MSG::SYSTEMPUB)	//公告
	{
		...
		shared_ptr<CUser> phUser = m_pRouter->GetUserManager()->Find(did);
...
// 		break;
	case ST_SYSTEM_LOG::ADD_FRIENDS:
		{
			pUser = pUserManager->Find(m_pRecvHead->did, TRUE);
			if (pUser == NULL)
				return RESULT_FAILED;
