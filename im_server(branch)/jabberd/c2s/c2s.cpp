...
typedef void (*generatepkt)(uint32 fd,char *username,char *pwd,uint32 src_ip, uint16 src_port,uint16 cmd,
                            uint32 fromid,uint32 toid,uint32 deviceId,RecvDataStruct *RecvData,void *input);
#else
typedef void (*generatepkt)(uint32 fd,char *username,char *pwd,uint32 src_ip, uint16 src_port,uint16 cmd,
...
// 解析imserver线程发过来的线程流
unsigned int ParseStream(c2s_t c2s, char *pData, sess_t sess, int len);
// 登录请求xml格式输出
unsigned int SendXmlLogin(c2s_t c2s, XT_LOGIN_ACK* login_ack, sess_t sess, uint32 deviceId);
// 聊天消息转发
unsigned int SendXmlMessage(c2s_t c2s, char* data, sess_t sess);
// 离线消息转发
unsigned int SendXmlOfflineMessage(c2s_t c2s, char* data, sess_t sess);
// 获取完整信息
unsigned int SendXmlFullUserInfo(c2s_t c2s, char* data);
// 获取分流结果
unsigned int SendXmlFlow(c2s_t c2s, char* data);
// 获取最近联系商家
unsigned int SendXmlWebList(c2s_t c2s, char* data);
// 心跳
unsigned int SendXmlHeartBeat(c2s_t c2s, char* data);
//超时
unsigned int SendXmlTimeout(c2s_t c2s, char* data);
...
//<iq xmlns='jabber:client' to='imserver.vm04.fn.com' id='sd12' type='result'><userid>5003133</userid></iq>
    ...
    */
    ...
    c2s->m_pXmppSession->Del(sess->m_selfID,0);
    c2s->m_pXmppSession->Add(sess->m_selfID,0,sess);
...
void _authreg_auth_set(c2s_t c2s, sess_t sess, nad_t nad)
{
    ...
    char username[1024], resource[1024], str[1024];
    int ar_mechs;
...
//(uint32 fd,char *username,char *pwd,uint32 unsigned src_ip, uint32 src_port,uint32 cmd,RecvDataStruct *RecvData)
    ...
    uint32 ipaddr = inet_addr(sess->ip);
#ifdef IM_USE_MULTIDEVICE    
    ((generatepkt)c2s->m_generatepkt)((uint32)sess->fd->fd,username,str,ipaddr,sess->port,(uint32)0x211,0,0,1,&packets,NULL);
#else
	((generatepkt)c2s->m_generatepkt)((uint32)sess->fd->fd,username,str,ipaddr,sess->port,(uint32)0x211,0,0,&packets,NULL);
#endif
...
//              if(sessCl != NULL)
//              {
                ParseStream(sess->c2s, buf->data+8, sessCl, len-8);
//              }
...
#ifdef IM_USE_MULTIDEVICE
                ((generatepkt)sess->c2s->m_generatepkt)((uint32)sess->fd->fd,NULL,NULL,ipaddr,sess->port,(uint32)CMD_P2PMSG_SEND,sess->m_selfID,dst_userid,1,&packets,&input);
#else
                ((generatepkt)sess->c2s->m_generatepkt)((uint32)sess->fd->fd,NULL,NULL,ipaddr,sess->port,(uint32)CMD_P2PMSG_SEND,sess->m_selfID,dst_userid,&packets,&input);
#endif
...

            {
                ...
                */
...
#ifdef IM_USE_MULTIDEVICE
                    ((generatepkt)sess->c2s->m_generatepkt)((uint32)sess->fd->fd,NULL,NULL,ipaddr,sess->port,(uint32)CMD_GET_SUB_ACCOUNT_REQ,sess->m_selfID,0,1,&packets,&subaccount_req);
#else
					((generatepkt)sess->c2s->m_generatepkt)((uint32)sess->fd->fd,NULL,NULL,ipaddr,sess->port,(uint32)CMD_GET_SUB_ACCOUNT_REQ,sess->m_selfID,0,&packets,&subaccount_req);
#endif
...
#ifdef IM_USE_MULTIDEVICE
                    ((generatepkt)sess->c2s->m_generatepkt)((uint32)sess->fd->fd,NULL,NULL,ipaddr,sess->port,(uint32)CMD_USERINFO_GET_REQ,sess->m_selfID,req.fid,1,&packets,&req);
#else
					((generatepkt)sess->c2s->m_generatepkt)((uint32)sess->fd->fd,NULL,NULL,ipaddr,sess->port,(uint32)CMD_USERINFO_GET_REQ,sess->m_selfID,req.fid,&packets,&req);
#endif
...
#ifdef IM_USE_MULTIDEVICE
                    ((generatepkt)sess->c2s->m_generatepkt)((uint32)sess->fd->fd,NULL,NULL,ipaddr,sess->port,(uint32)CMD_DIRECTORY_REQ,sess->m_selfID,req.id,1,&packets,&req);
#else
					((generatepkt)sess->c2s->m_generatepkt)((uint32)sess->fd->fd,NULL,NULL,ipaddr,sess->port,(uint32)CMD_DIRECTORY_REQ,sess->m_selfID,req.id,&packets,&req);
#endif
...
#ifdef IM_USE_MULTIDEVICE
                    ((generatepkt)sess->c2s->m_generatepkt)((uint32)sess->fd->fd,NULL,NULL,ipaddr,sess->port,(uint32)CMD_XMPP_BUSI_LIST_REQ,sess->m_selfID,0,1,&packets,&req);
#else
                ...
                else if((ns = nad_find_scoped_namespace(nad, uri_HISTORYMSG, NULL)) >= 0 && (query = nad_find_elem(nad, 0, ns, "query", 1)) >= 0)
                {
                    ...
                    uint32 ipaddr = inet_addr(sess->ip);
#ifdef IM_USE_MULTIDEVICE                    
                    ((generatepkt)sess->c2s->m_generatepkt)((uint32)sess->fd->fd,NULL,NULL,ipaddr,sess->port,(uint32)CMD_HISTORY_MSG_REQ,sess->m_selfID,0,1,&packets,&req);
#else
...
int _c2s_client_mio_callback(mio_t m, mio_action_t a, mio_fd_t fd, void *data, void *arg)
{
    ...
    switch(a)
    {
        ...
        case action_CLOSE:
...
#ifdef IM_USE_MULTIDEVICE               
            ((generatepkt)sess->c2s->m_generatepkt)((uint32)sess->fd->fd,NULL,NULL,ipaddr,sess->port,(uint32)CMD_LOGOUT,sess->m_selfID,req.id,1,&packets,&req);
#else
            ...
            sess->c2s->m_pXmppSession->Del(sess->m_selfID,0, FALSE);
...
//                    sm_end(tsess, tres);
            ...
            if(smid >= 0)
                for(bres = sess->resources; bres != NULL; bres = bres->next)
                {
                    if(bres->sm_id[0] == '\0' || ((int)strlen(bres->sm_id) == NAD_AVAL_L(nad, smid) && strncmp(bres->sm_id, NAD_AVAL(nad, smid), NAD_AVAL_L(nad, smid)) == 0))
                        break;
                }
            if(bres == NULL)
            {
...
//                sm_end(sess, tres);
            ...
            if(NAD_ENS(nad, 1) == ns && action >= 0)
            {
                ...
                if(id < 0 || bres->sm_request[0] == '\0' || (int)strlen(bres->sm_request) != NAD_AVAL_L(nad, id) || strncmp(bres->sm_request, NAD_AVAL(nad, id), NAD_AVAL_L(nad, id)) != 0)
                {
                    if(id >= 0)
                    {
...
// 解析imserver线程发过来的线程流
unsigned int ParseStream(c2s_t c2s, char *pData, sess_t sess, int datalen)
{
    ...
    switch (head->cmd)
    {
        case CMD_LOGIN_ACK:
        {
            ...
            if(sess != NULL)
#ifdef IM_USE_MULTIDEVICE            
                len = SendXmlLogin(c2s, login_ack, sess, head->deviceId);
#else
				len = SendXmlLogin(c2s, login_ack, sess, 0);
#endif
        }
        break;
        /*
        ...
        case CMD_SERVERMSG_IND:
        {
            ...
            switch(server_transmit->msgtype)
            {
                case CMD_P2PMSG_SEND:
                    len = SendXmlMessage(c2s, pData, sess);
                    break;
                case CMD_P2PMSG_SEND_OFFLINE:
                    len = SendXmlOfflineMessage(c2s, pData, sess);
                /*
        ...
        */
        ...
        {
            SendXmlWebList(c2s, pData);
        }
        break;

        case CMD_GET_SUB_ACCOUNT_ACK:
        {
            SendXmlFlow(c2s, pData);
        }
        break;
        case CMD_DIRECTORY_ACK:
        {
            SendXmlHeartBeat(c2s, pData);
        }
        break;
        /*
...
unsigned int SendXmlLogin(c2s_t c2s, XT_LOGIN_ACK* login_ack, sess_t sess, uint32 deviceId)
{
    ...
    if(login_ack->ret == 0)
    {
        ...
        sess->c2s->m_pXmppSession->Del(login_ack->user_id,deviceId);
        sess->c2s->m_pXmppSession->Add(login_ack->user_id,deviceId,sess);
    ...
    else
    {
        ...
        sess->c2s->m_pXmppSession->Del(login_ack->user_id,0);
        sess->c2s->m_pXmppSession->Add(login_ack->user_id,0,sess);
        log_debug(ZONE, "add map userid %u --- > fd %u",login_ack->user_id,sess->fd->fd);
...
unsigned int SendXmlMessage(c2s_t c2s, char* data, sess_t sess)
{
    ...
    sess = (sess_t)c2s->m_pXmppSession->GetSess(msg.to_id,0);
    if(sess != NULL)
    {
...
unsigned int SendXmlFullUserInfo(c2s_t c2s, char* data)
{
    ...
    // 查找sess
    sess_t sess = NULL;
    sess = (sess_t)c2s->m_pXmppSession->GetSess(head->did,0);
    if(sess != NULL)
    {
...
unsigned int SendXmlFlow(c2s_t c2s, char* data)
{
    ...
    log_debug(ZONE, "SendXmlFlow recv:id:%d, email:%s, username:%s, mobile:%s, nickname:%s, usersign:%s, merchantid:%d"
              , ack.id, ack.strEmail, ack.strUsername, ack.strMobile, ack.strNickname, ack.usersign, ack.merchant_id);
    ...
    nad_append_elem(sess->result, ns, "username", 2);
    nad_append_cdata_gbk(sess->result,ack.strUsername, strlen(ack.strUsername), 3);

    nad_append_elem(sess->result, ns, "mobile", 2);
    nad_append_cdata_gbk(sess->result,ack.strMobile, strlen(ack.strMobile), 3);

    nad_append_elem(sess->result, ns, "nickname", 2);
    nad_append_cdata_gbk(sess->result,ack.strNickname, strlen(ack.strNickname), 3);

    nad_append_elem(sess->result, ns, "usersign", 2);
    nad_append_cdata_gbk(sess->result,ack.usersign, strlen(ack.usersign), 3);

    nad_append_elem(sess->result, ns, "merchantid", 2);
    memset(szTemp, 0, 1024);
    sprintf_s(szTemp, "%d", ack.merchant_id);
    nad_append_cdata_gbk(sess->result,szTemp, strlen(szTemp), 3);
...
unsigned int SendXmlHeartBeat(c2s_t c2s, char* data)
{
    ...
    config_elem_t elem;
    elem = config_get(sess->c2s->config, "local.id");
    if(elem != NULL)
    {
...
unsigned int SendXmlTimeout(c2s_t c2s, char* data)
{
    ...
    // 查找sess
    if(c2s->m_pXmppSession->IsExist(head->did,0))
    {
        c2s->m_pXmppSession->Del(head->did,0);
