...
/** generate a new_cplus session request id */
...
static nad_t _sm_build_route(sess_t sess, bres_t res, const char *action, const char *target, const char *id) {
    ...
    log_debug(ZONE, "built new_cplus route nad for %s action %s target %s id %s", jid_full(res->jid), action, target, id);
