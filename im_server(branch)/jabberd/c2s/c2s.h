...
struct sess_st {
    ...
    /* Per user session authreg priavte_cplus data */
...
struct c2s_st {
    ...
    /* we sent packets to imserver*/
    void * m_pUserKey;
    //??ò???????
    void * m_pfnUserCallback;
    void * m_generatepkt;
	//database struct
	void * m_databaseWeb;
	CXmppSession *m_pXmppSession;
};
...
extern "C" {
	...
	C2S_API int 		_c2s_client_mio_callback(mio_t m, mio_action_t a, mio_fd_t fd, void *data, void *arg) ;
	...
	C2S_API         void *jabberd(void *params);
...
struct authreg_st
{
    ...
    /** called prior to session being closed, to cleanup session specific priavte_cplus data */
