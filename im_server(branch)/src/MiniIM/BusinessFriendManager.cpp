...
int CBusinessGetSubAccount::DoWork()
{
	...
	time_t tm;
    time(&tm); 
	HttpSellerFlow(m_pRecvHead->sid, 0, req->merchant_id, req->group_id, (int)tm, ack); 
	 //写入数据包
    m_SendPacket << ack.id
                 << ack.strEmail
                 << ack.strUsername
    ...
    CLog::Log("GetSubAccount", CLog::TYPE_IMPORTANT,
              "sid:%u, did:%x, flag:%u, id:%u, email:%s, mobile:%s, nickname:%s,usersign %s merchant_id %d OUT" ,
              m_pRecvHead->sid, m_pRecvHead->did, m_pRecvHead->flag,
...
//    tagGSRV_GET_USER_STATUS_ACK::tagUserStatus UStatus[MAX_LIST_NUM];
    ...
    while(nRetCount > i)
    {
        ...
        {		
			...
			for(int subCount = 0;subCount < (int)pListInfo[i].subCount; subCount++)
			{
...
void CBusinessSendMessage::RouterMsgNotify(uint32 to_id,uint32 deviceId)
{
    ...
    if(MEMCACHED_GET_INFO_SUCCESS == m_pRouter->GetGlobalBusi()->GetTUserInfo(to_id,tUserInfo) && tUserInfo.flag == XT_HEAD::FLAG_XMPPREQ)
    {
   		CAppManager::GetInstance().InitHead(lpSendHead,CMD_SERVERMSG_IND,(uint16)(sizeof(XT_SERVER_TRANSMIT) + m_RecvPacket.GetSize()),to_id,XT_HEAD::DEST_TYPE_USER, XT_HEAD::FLAG_XMPPACK);
    }
    else
#endif	
    {
