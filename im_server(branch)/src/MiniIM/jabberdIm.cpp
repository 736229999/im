#if defined XMPP_BUILD && defined ACCESS_SERVER
#include "jabberdIm.h"
#include <curl/curl.h>
#include "Log.h"
    ...
    switch(cb)
    {
        ...
        case sx_sasl_cb_CHECK_PASS:
            ...
            if(CheckBuyer(creds->authnid,creds->pass,guid,username,sess->ip))
            {
                ...
                // 登录前先删除原有的会话信息
                CAppManager::GetInstance().GetXmppSession()->Del(m_uid,0);
                ...
                ((generatepkt)c2s->m_generatepkt)((uint32)sess->fd->fd,(char *)creds->authnid,(char *)creds->pass,ipaddr,sess->port,(uint32)0x211,0,0,1,&packets,NULL);
#else
        ...
        case sx_sasl_cb_CHECK_MECH:
            ...
            i=0;
            while(i<(int)sizeof(mechbuf) && mech[i]!='\0')
            {
...
//  m_pDataBaseWeb = &DatabaseWeb;
    ...
    c2s->m_pXmppSession = CAppManager::GetInstance().GetXmppSession();
