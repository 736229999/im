...
bool CUserManager::UserLogoutLockHandle(uint32 userId, LOGOUT_LOCK_HANDLE_TYPE handleType)
{
	switch(	handleType )
	{
		case LOGOUT_LOCK_ADD:
			return m_pGlobalBusi->AddLogoutLock(userId);
		case LOGOUT_LOCK_GET:
			return m_pGlobalBusi->GetLogoutLock(userId);
		case LOGOUT_LOCK_DEL:
			return m_pGlobalBusi->DelLogoutLock(userId);
		default:
			CLog::Log("UserLogoutLockHandle", CLog::TYPE_ERROR, "typeError");
			return false;
	}
}
...
bool CUserManager::AddUserToRedis(const shared_ptr<CUser> &pUser)
{
	...
	return m_pGlobalBusi->AddModiUserInfo(pUser.get(),tagGSRV_ADDMODIFY_FULLUSERINFO_QUERY::ACTION_ADD,0);
}
...
#else
bool CUserManager::Del(uint32 userid,uint32 deviceid)
{
	if(m_pGlobalBusi == NULL)
	{
		return false;
...
bool CUserManager::GetGroupList(uint32 uid, vector<XT_GROUPINFO_GET_ACK::tagGroupInfo> &vecGroupInfo)
{
	...
	CHttpClient httpClient;
	httpClient.SetDebug(true);
	string strgrouplisturl(CAppManager::GetInstance().GetConfig().GetFriendGroupListUrl());
