...
//	uint8	m_nLevel;								//用户等级
	...
	static bool g_cs_exist(uint32 cs_id, CGlobalBusi *glob_busi);
	// 查找客服是否正在被监控，如果被监控，返回主管id(先查本机，再查memcache)
	static bool g_cs_ismonitored(uint32 cs_id, bool &monitored, uint32 &cs_master_id, CGlobalBusi *glob_busi);
	// 查找客服正在接待的买家(先查本机，再查memcache)
	static bool g_cs_get_chatting_buyer(uint32 cs_id, uint32 &chatting_buyer_id, CGlobalBusi *glob_busi);
	// 设置客服正在被谁监控(同时修改上下文及memcache)
	static bool g_cs_set_director(uint32 cs_id, uint32 cs_master_id, CGlobalBusi *glob_busi);
	// 设置客服正在联系的买家(同时修改上下文及memcache)
	static bool g_cs_set_chatting_buyer(uint32 cs_id, uint32 chattring_buyer_id, CGlobalBusi *glob_busi);
	// 获取客服名字
	static bool g_cs_get_name(uint32 cs_id, char name[], CGlobalBusi *glob_busi);
