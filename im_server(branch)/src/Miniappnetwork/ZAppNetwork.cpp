	...
	else*/ if(pHead->dtype == XT_HEAD::DEST_TYPE_USER )
	{
		...
		//CLog::Log("SendDataToA",CLog::TYPE_DEBUG,"2sid:%d, from:%d, cmd:0x%.4x, did:%d, dtype:%d",
			...
			{//直连用户，则直接发给客户端
				...
				{
#ifdef XMPP_BUILD				
					if(tUserInfo.flag != XT_HEAD::FLAG_MOBREQ && tUserInfo.flag != XT_HEAD::FLAG_XMPPREQ)
					{	// xmpp数据不加密
#else
...
#ifdef XMPP_BUILD							
						else if(pHead->flag == XT_HEAD::FLAG_XMPPACK)	
						{}
#endif						
						else
							pHead->flag = XT_HEAD::FLAG_MOBSRVACK;	
...
*/
		...
		{
			CLog::Log("CZAppNetwork::SendDataToA",CLog::TYPE_ERROR,"userid[%u] not exist", pHead->did);	
			//有ip则直接发送
			if( ip != 0 )
			{
				sendData.dest_ip = ip;
