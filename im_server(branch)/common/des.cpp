...
char HexToInt(const char* pHex)
{
	int nChar = 0;

	int msb = toHexDigit( *pHex );
	int lsb = toHexDigit( *(pHex+1) );
	if (msb != -1 && lsb != -1)
	{
		nChar = (msb << 4) + lsb;
	}
...
void StringToHex(const char* pStr,size_t nLength,char* pOutData,int& nOutBuf)
{
	size_t nOutSize = 0;
	for( size_t i = 0 ; i < nLength ; i+=2)
	{
		if( (size_t)nOutBuf < (i+1))
		{
			break;
		}

		*(pOutData+i/2) = HexToInt(pStr+i);
		++nOutSize;
	}
...
void HexToString( unsigned char *pData,size_t nLength, unsigned char *strOut)
{
	char* pszHex = new char[2*nLength + 1];
	if (!pszHex)
	{
		//IBERROR("memory not enougth!");
		return;
	}

	for( size_t i = 0 ; i < nLength ; ++i)
	{
		sprintf(pszHex + 2*i,"%.2x",(unsigned char)pData[i]);
	}

	pszHex[2*nLength] = '\0';

	strcpy((char *)strOut, pszHex);

	delete[] pszHex;
}

static int pad80(unsigned char *DataIn, int *iDataLen)
{
	int byMod8 = *iDataLen % 8;

/*
	DataIn[*iDataLen] = 0x80;
	*iDataLen = *iDataLen + 1;	//增加一个的长度
...
// 适应JH加密修改
	int byTempLen = *iDataLen;
	memset(DataIn + byTempLen, 8-byMod8, 8-byMod8);  *iDataLen += (8-byMod8);
	return 0;
}
...
void DesEnc(unsigned char *outdata, unsigned char *indata, int datalen, const unsigned char *key)
{
	...
	unsigned char soutdata[128]={0};

	// move data
	memcpy(Key, key, 8);
	memcpy(temp, indata, datalen);
...
//适应JH加密修改
	...
	blklen = datalen / 8;
	//	for(i = 0; i < blklen; i++, indata += 8, soutdata += 8)
	//		endes(indata , Key , soutdata);
	for(i = 0; i < blklen; i++)
	{
		endes(temp + i * 8, Key, soutdata + i * 8);
	}

	HexToString(soutdata,datalen,outdata);
}

void DesDec(unsigned char *outdata, unsigned char *sindata, int sdatalen, const unsigned char *key)
{
	int i, blklen;
	unsigned char Key[8] = 
	{
	...
	StringToHex((char *)sindata, sdatalen, indata, datalen);
	...
	blklen = datalen / 8;
	//	for(i = 0; i<blklen; i++, temp += 8, outdata += 8)
	//		undes(temp , Key , outdata);
	for(i = 0; i < blklen; i++)
	{
		undes(temp + i * 8, Key, outdata + i * 8);
	}
	unsigned char cFill = outdata[blklen*8-1] ;
	if( (cFill>0 && cFill<=8)  || cFill==0x80 )
	{
		if(outdata[blklen*8-1] == outdata[blklen*8-2] )
		{
			for( i = blklen*8-1 ; i >= (blklen-1)*8 ;--i)
			{
				if( outdata[i] == cFill )
				{
					outdata[i]='\0';
