...
CConfigInfo::CConfigInfo(void)
{
	...
	m_szFriendGroupListUrl[0] = '\0';

	m_szUserOfflineMSG[0] = '\0';

	m_szAddGroupUrl[0] = '\0';
	m_szModGroupUrl[0] = '\0';
	m_szDelGroupUrl[0] = '\0';

	m_szAddFriendUrl[0] = '\0';
	m_szModFriendGroupUrl[0] = '\0';
	m_szDelFriendUrl[0] = '\0';

	m_szGetRecentSubMerchantUrl[0] = '\0';
	m_szSellerFlowUrl[0] = '\0';
}
...
// 获取好友分组的url
char* CConfigInfo::GetFriendGroupListUrl()
{
	if( m_szFriendGroupListUrl[0] == '\0')	
	{
		strncpy_s(m_szFriendGroupListUrl,
			m_IniFile.GetString(CONFIG_SECTION_IMAPI,
			CONFIG_KEY_IM_GET_GROUPLIST_URL,""),
			sizeof(m_szFriendGroupListUrl));
	}
	return m_szFriendGroupListUrl;	 
}

// 获取 用户离线消息
char* CConfigInfo::GetUserOfflineMSGUrl()
{
	if( m_szUserOfflineMSG[0] == '\0')	
	{
		strncpy_s(m_szUserOfflineMSG,
			m_IniFile.GetString(CONFIG_SECTION_IMAPI,
			CONFIG_KEY_IM_GET_OFFLINEMSG_URL,""),
			sizeof(m_szUserOfflineMSG));
	}
	return m_szUserOfflineMSG;	 
}

// 添加分组url
char* CConfigInfo::GetAddGroupUrl()
{
	if( m_szAddGroupUrl[0] == '\0')	
	{
		strncpy_s(m_szAddGroupUrl,
			m_IniFile.GetString(CONFIG_SECTION_IMAPI,
			CONFIG_KEY_IM_ADDGROUP,""),
			sizeof(m_szAddGroupUrl));
	}
	return m_szAddGroupUrl;	 
}

// 修改分组url
char* CConfigInfo::GetModGroupUrl()
{
	if( m_szModGroupUrl[0] == '\0') 
	{
		strncpy_s(m_szModGroupUrl,
			m_IniFile.GetString(CONFIG_SECTION_IMAPI,
			CONFIG_KEY_IM_MODGROUP,""),
			sizeof(m_szModGroupUrl));
	}
	return m_szModGroupUrl;  
}

// 删除分组url
char* CConfigInfo::GetDelGroupUrl()
{
	if( m_szDelGroupUrl[0] == '\0') 
	{
		strncpy_s(m_szDelGroupUrl,
			m_IniFile.GetString(CONFIG_SECTION_IMAPI,
			CONFIG_KEY_IM_DELGROUP,""),
			sizeof(m_szDelGroupUrl));
	}
	return m_szDelGroupUrl;  
}

// 添加好友url
char* CConfigInfo::GetAddFriendUrl()
{
	if( m_szAddFriendUrl[0] == '\0') 
	{
		strncpy_s(m_szAddFriendUrl,
			m_IniFile.GetString(CONFIG_SECTION_IMAPI,
			CONFIG_KEY_IM_ADDFRIEND,""),
			sizeof(m_szAddFriendUrl));
	}
	return m_szAddFriendUrl;  
}

// 修改好友分组url
char* CConfigInfo::GetModFriendGroupUrl()
{
	if( m_szModFriendGroupUrl[0] == '\0') 
	{
		strncpy_s(m_szModFriendGroupUrl,
			m_IniFile.GetString(CONFIG_SECTION_IMAPI,
			CONFIG_KEY_IM_MODFRIENDGROUP,""),
			sizeof(m_szModFriendGroupUrl));
	}
	return m_szModFriendGroupUrl;  
}

// 删除好友url
char* CConfigInfo::GetDelFriendUrl()
{
	if( m_szDelFriendUrl[0] == '\0') 
	{
		strncpy_s(m_szDelFriendUrl,
			m_IniFile.GetString(CONFIG_SECTION_IMAPI,
			CONFIG_KEY_IM_DELFRIEND,""),
			sizeof(m_szDelFriendUrl));
	}
	return m_szDelFriendUrl;  
}

// 获取最近联系商家url
char* CConfigInfo::GetRecentSubMerchantUrl()
{
	if( m_szGetRecentSubMerchantUrl[0] == '\0') 
	{
		strncpy_s(m_szGetRecentSubMerchantUrl,
			m_IniFile.GetString(CONFIG_SECTION_IMAPI,
			CONFIG_KEY_IM_GET_RECENT_SUBMERCHENT,""),
			sizeof(m_szGetRecentSubMerchantUrl));
	}
	return m_szGetRecentSubMerchantUrl; 
}

// 商家分流url
char* CConfigInfo::GetSellerFlowUrl()
{
	if( m_szSellerFlowUrl[0] == '\0') 
	{
		strncpy_s(m_szSellerFlowUrl,
			m_IniFile.GetString(CONFIG_SECTION_IMAPI,
			CONFIG_KEY_IM_SELLERFLOW,""),
			sizeof(m_szSellerFlowUrl));
	}
	return m_szSellerFlowUrl; 
}