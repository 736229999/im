...
#ifndef GLOBAL_CACHED
#define GLOBAL_CACHED
#pragma once
...
namespace global_cache
{
    ...
    public:
        ...
        global_Cache(char *hostname, int port)
        {
...
//              value_length = strlen(integer);
		...
		bool getset(const char *key, const char* value,char *oldVal)
		{
			redisReply *reply = NULL;
            size_t value_length= 0;
			reply = (redisReply *)redisClusterCommand(cc, "getset %s %s", key,value);
			if(reply == NULL)
			{
				printf("reply is null[%s]\n", cc->errstr);
				//redisClusterFree(cc);
			...
			printf("getset %s value %s reply->type:%d integer %lld\r\n",key,value,reply->type,reply->integer);
			if(reply->type == REDIS_REPLY_STRING)
            {
                value_length = strlen(reply->str);
                if (reply->str != NULL && value_length > 0 && value_length < MAX_GET_LENGTH)
                {
                    strcpy(oldVal,reply->str);
                }
                else
                {
                    freeReplyObject(reply);
            ...
            {
                char integer[64] = {0};
                sprintf(oldVal,"%lld",reply->integer);
                strcpy(oldVal,integer);
            }
            else if(reply->type == REDIS_REPLY_NIL)
            {
                freeReplyObject(reply);
...
//        size_t value_length= 0;
        ...
        bool zadd(const char *key, const uint32 score,const char* member)
        {
            //size_t value_length= 0;
            redisReply *reply = NULL;
            reply = (redisReply *)redisClusterCommand(cc, "zadd %s %u %s", key,score,member);
            if(reply == NULL)
            {
                printf("reply is null[%s]\n", cc->errstr);
                //redisClusterFree(cc);
            ...
            printf("ZADD reply->type:%d key %s score %u,member %s\r\n", reply->type,key,score,member);
            if(reply->type == REDIS_REPLY_INTEGER)
            {
                freeReplyObject(reply);
                return true;
		...
		//这里member和score是相同的数值
        bool zadd(const char *key,const char* member)
        {
            //size_t value_length= 0;
            redisReply *reply = NULL;
            reply = (redisReply *)redisClusterCommand(cc, "zadd %s %s %s", key,member,member);
            if(reply == NULL)
            {
                printf("reply is null[%s]\n", cc->errstr);
                //redisClusterFree(cc);
            ...
            printf("ZADD reply->type:%d key %s score %s,member %s\r\n", reply->type,key,member,member);
            if(reply->type == REDIS_REPLY_INTEGER)
            {
                freeReplyObject(reply);
                return true;
        ...
        bool zrem(const char *key,const char *member)
        {
            //size_t value_length= 0;
            redisReply *reply = NULL;
            reply = (redisReply *)redisClusterCommand(cc, "zrem %s %s", key,member);
            if(reply == NULL)
            {
                printf("reply is null[%s]\n", cc->errstr);
                //redisClusterFree(cc);
            ...
            printf("ZREM reply->type:%d key  %s member %s,return %lld\r\n", reply->type,key,member,reply->integer);
            if(reply->type == REDIS_REPLY_INTEGER)
            {
                freeReplyObject(reply);
                return true;
        ...
        bool zcard(const char *key,uint64_t *length)
        {
            //size_t value_length= 0;
            redisReply *reply = NULL;
            reply = (redisReply *)redisClusterCommand(cc, "zcard %s", key);
            if(reply == NULL)
            {
                printf("reply is null[%s]\n", cc->errstr);
                //redisClusterFree(cc);
            ...
            printf("ZCARD reply->type:%d key  %s  length %lld\r\n", reply->type,key,reply->integer);
            if(reply->type == REDIS_REPLY_INTEGER)
            {
                *length =(uint64_t)reply->integer;
                printf(" ---LLEN reply->type:%d key  %s  value %lld *value %ld\r\n", reply->type,key,reply->integer,*length);
        ...
        bool zrange(const char *key, uint32 start, uint32 stop,int &count,char (*ret_val)[MAX_REDIS_LENGTH])
        {
            size_t value_length= 0;
            redisReply *reply = NULL;
            reply = (redisReply *)redisClusterCommand(cc, "zrange %s %d %d", key,start,stop);
            int countx = 0;
            if(reply == NULL)
            {
                printf("reply is null[%s]\n", cc->errstr);
            ...
            printf("ZRANGE reply->type:%d key %s value %lld\r\n", reply->type,key,reply->integer);
            if (reply->type == REDIS_REPLY_ARRAY)
            {
                size_t redis_len = 0;
				printf("ZRANGE elements %ld\r\n", reply->elements);
...
//					printf("type %d \n",reply->element[j]->type);
					if(reply->element[j]->type == REDIS_REPLY_INTEGER)
		            {
	                    ...
	                    printf("type %d value %s len %ld\n",reply->element[j]->type,reply->element[j]->str,redis_len);
		                sprintf(integer,"%lld",reply->integer);
		                strcpy(ret_val[j],integer);						
	                    countx++;
		            }
        ...
        bool zrange(const char *key, int start, int stop,int &count,char **ret_val, int nValMaxLength)
        {
            size_t value_length= 0;
            redisReply *reply = NULL;
            reply = (redisReply *)redisClusterCommand(cc, "zrange %s %d %d", key,start,stop);
...
//					printf("type %d \n",reply->element[j]->type);
					if(reply->element[j]->type == REDIS_REPLY_INTEGER)
		            {
	                    ...
	                    printf("type %d value %s len %ld\n",reply->element[j]->type,reply->element[j]->str,redis_len);
		                sprintf(integer,"%lld",reply->integer);
		                if((int)strlen(integer) < nValMaxLength)
		                {
		                	strcpy(ret_val[j],integer);						
	                    }
	                    countx++;
					...
					else if(reply->element[j]->type == REDIS_REPLY_STRING)
					{
	                    ...
	                    if(redis_len > 0 && redis_len < MAX_GET_LENGTH && (int)redis_len < nValMaxLength)
	                    {
	                        strcpy(ret_val[j],reply->element[j]->str);
	                    }						
	                    countx++;
            ...
            {
                value_length = strlen(reply->str);
                if (reply->str != NULL && value_length > 0 && value_length < MAX_GET_LENGTH && (int)value_length < nValMaxLength)
                {
                    strcpy(ret_val[0],reply->str);
                }
                else
        ...
        bool redisGetAllKey(const char *key, vector<string> &vecKey)
        {
        	redisReply *reply = NULL;
            reply = (redisReply *)redisClusterCommand(cc, "hscan %s 0 count 1000", key);
            if(reply == NULL)
            {
                printf("redisGetAllKey reply is null[%s]\n", cc->errstr);
                //redisClusterFree(cc);
                return false;
            }
            printf("hscan %s reply->type:%d\r\n",key,reply->type);
			...
			if (reply->type == REDIS_REPLY_ARRAY)
            {
				...
				redisReply *replykey = reply->element[1];
            	if(replykey->type != REDIS_REPLY_ARRAY)
            	{
            		freeReplyObject(reply);
            		return false;
            	}

				printf("hscan element count:%ld\n",replykey->elements);
                for (size_t j = 0; j < replykey->elements; j+=2)
                {
                	printf("hscan element type:%d\n",replykey->element[j]->type);
                    if(replykey->element[j]->type == REDIS_REPLY_STRING )
                    {
                        printf("hscan idx %ld element %s elementlen %d\n", j, replykey->element[j]->str,replykey->element[j]->len);
                        vecKey.push_back(replykey->element[j]->str);
                    }
                }
                freeReplyObject(reply);
            	return true;
...
namespace global_cache
{
    ...
    public:
        ...
        global_Cache(const std::string &config)
        {
            memc_= memcached(config.c_str(), config.size());
        }

        global_Cache(const std::string &hostname, in_port_t port)
        {
            memc_= memcached(NULL, 0);
            if (memc_)
            {
        ...
        global_Cache(memcached_st *clone)
        {
            memc_= memcached_clone(NULL, clone);
        }

        global_Cache(const global_Cache &rhs)
        {
            memc_= memcached_clone(NULL, rhs.getImpl());
        }

        global_Cache &operator=(const global_Cache &rhs)
        {
            if (this != &rhs)
            {
