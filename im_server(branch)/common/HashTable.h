...
public:
	...
	// 提供几个接口用于遍历
	// 获取hash分支数
	int GetHashElem()	{	return m_hashElem;	}
	// 获取单个hash分支下面的所有索引id
	void GetElemIDs(int nElemIdex, std::vector<uint32> &vecID)
	{
		vecID.clear();
		if(nElemIdex < 0 || nElemIdex >= m_hashElem)
			return;

		tagLink * pNode;
		CAutoLock MyLock(m_tableLock[nElemIdex]);

		pNode = m_hash[nElemIdex];
		while(pNode)
		{
			vecID.push_back(pNode->id);
			pNode = pNode->next;
		}
	}
