...
{
	uint8     login_flag    ; //78两位为登陆方式，6位是卖家或买家//5.6位是卖家买家客服登录 //4位用于判断是否校验异地登录
	enum{
		LOGINBYID = 0,
		LOGINBYNAME = 1,
		LOGINBYMOBILENO = 2,
...
{
	uint32		id;
	uint8 authData; //校验数据 = 0x00	客服彻底退出 0x01 客服接待退出，但不清除上下文；0x02 客服接待退出，但不通知客服

} XT_LOGOUT;

typedef struct XT_SET_WEB_CS_REQ
{
	uint32		id;
	uint8 authData; //类型 0x00 清除 0x01设置
	uint32		csId;
	uint8 		teamId; 
} XT_SET_WEB_CS_REQ;


typedef struct XT_CLEAR_CS_CONSULT_REQ
{
	uint32		type;
	uint32		clientId;
	uint32		csId;
	uint8 		teamId; 
} XT_CLEAR_CS_CONSULT_REQ;
...
typedef struct XT_SERVER_USERINFO_GET_ACK
{
	...
	char	avatoruri[MAX_AVATOR_LEN + 1];	//闪秀地址
	uint8   mobilestatus				;	//用户手机状态 0未绑定，1已绑定 2已绑定并且订制了业务
	uint32  integral					;	//用户积分		/*新定义：标记是否为主管*/
	uint32  csreceptionlimit			;	//客服最大接待量
	char	guid[MAX_GUID_LEN + 1];			// GUID	
	char	workGroup[MAX_WORKGROUP_LEN +1];			//工作组的用户组信息
...
typedef struct XT_WEB_BUSI_LAST_MSG_REQ
{
	uint32 id;//获取列表的id号
	uint32 msgnum;//请求的消息的条数
}XT_WEB_BUSI_LAST_MSG_REQ,*LPXT_WEB_BUSI_LAST_MSG_REQ;

typedef struct XT_WEB_BUSI_LAST_MSG_ACK
{
	uint8 ret;
	uint32 id;//id
	uint32 msgnum;//请求的消息的条数	
	...
	struct BusiLastList
	{
		...
		uint32 lastMsgTime;
		char nickname[MAX_NICKNAME_LEN+1];
		char lastMsg[MAX_MSG_LENGTH+1];		
	}busiLastList[1];
}XT_WEB_BUSI_LAST_MSG_ACK,*LPXT_WEB_BUSI_LAST_MSG_ACK;
...
typedef struct XT_GET_SUB_ACCOUNT_REQ
{
	...
	uint8       type;							//分组分流or在线分流or 客服

	enum{ONLINE = 0,TEAM, CS};
...
// 客服工作台版本，从运营后台获取得组织成员
typedef struct XT_CS_INFO
{
	uint32 	id;// USERID
	uint32  csid;//运营后台csid
	char 	username[MAX_USERNAME_LEN+1];//哞哞登录名
	uint8 	permission;//哞哞账号权限：0代表员工，1代表主管
	char 	importname[MAX_USERNAME_LEN+1];//导入人员
	uint32  importtime;
	uint8 	importgroup;//导入人员组织
	char 	realname[MAX_USERNAME_LEN+1];//账号所属人员
	uint8 	csGroupId;//账号所属组织
	uint8 	status;//1代表启用，0代表停用，2代表全部
	uint8 	receptionlimit;//接待上限
	char    organName[100];
	XT_CS_INFO ()
	{
		memset(this,0,sizeof(XT_CS_INFO));
	}
}XT_CS_INFO, * LPXT_CS_INFO;
//用户修改登录密码请求
typedef struct XT_IMPORT_CS_REQ
{
	uint32  num;    
	struct TagCSInfo
	{
		...
		char 	username[MAX_USERNAME_LEN+1];//哞哞登录名
		uint8 	permission;//哞哞账号权限：0代表主管，1代表员工
		uint8 	receptionlimit;//接待上限
	...
	XT_IMPORT_CS_REQ()
	{
		memset(this,0,sizeof(XT_IMPORT_CS_REQ));
	}
} XT_IMPORT_CS_REQ, *LPXT_IMPORT_CS_REQ;
...
{
	uint8	bAdd;		// 1：增加成员	0：减少成功 2:排队队列异动
	uint32	idBuyer;	// 买家ID	
	uint32 	teamId;		//所在组ID
	uint32  QueueLength;
	char    nicknameBuyer[MAX_NICKNAME_LEN+1] ;// 买家昵称
	
	enum { TYPE_ADD = 0,TYPE_DELETE};
	XT_WAITQUEU_CHANGE()
	{
...
typedef struct XT_BUYERINFO_CHANGE
{
	uint32	idBuyer;	// 买家ID	
	uint32 	teamId;		//所在组ID
	uint32  idSeller;
	//uint32 	loginIP;	// 买家登录的webim ip	
	
	XT_BUYERINFO_CHANGE()
	{
		memset(this, 0, sizeof(XT_BUYERINFO_CHANGE));
	}
}XT_BUYERINFO_CHANGE, *LPXT_BUYERINFO_CHANGE;


// 强制下线客服结构体
typedef struct XT_FORCE_OFFLINE_CS
{
	uint32 idCs;		// 客服ID

	XT_FORCE_OFFLINE_CS()
	{
		memset(this, 0, sizeof(XT_FORCE_OFFLINE_CS));
	}
}XT_FORCE_OFFLINE_CS, *LPXT_FORCE_OFFLINE_CS;
...
{
	uint8		type;			//请求类型  1、主动接待 2、退出接待3.自动接待
	uint32 	 teamId;	//所在组ID
	uint32	idBuyer;	// 买家ID	

	enum { TYPE_OFFER = 0,TYPE_QUIT,TYPE_AUTO_OFFER,TYPE_IND_QUEUE};
	XT_RECEPTE_CUST_REQ()
	{
...
// WEB查询是否接待相关协议（客户端发送给服务端的协议）
//#define CMD_RECEPTE_CUST_REQ	0x8110		// 接待客户请求
typedef struct XT_RECEPTE_CHECK_REQ
{
	uint32		merchant_id;					// 商家id
	uint32 		group_id;						// 商家分组id
	uint32		buyer_id;						//买家id
	uint8       type;							//分组分流or在线分流

	XT_RECEPTE_CHECK_REQ()
	{
		memset(this, 0, sizeof(XT_RECEPTE_CHECK_REQ));
	}
}XT_RECEPTE_CHECK_REQ, *LPXT_RECEPTE_CHECK_REQ;
//#define CMD_RECEPTE_CUST_ACK	0x8111		// 接待客户应答
typedef struct XT_RECEPTE_CHECK_ACK
{
	uint8      ret;
	uint32	   buyer_id;
	XT_RECEPTE_CHECK_ACK()
	{
		memset(this, 0, sizeof(XT_RECEPTE_CHECK_ACK));
	}
}XT_RECEPTE_CHECK_ACK, *LPXT_RECEPTE_CHECK_ACK;



/*vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv设置客服聊天买家vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv*/
//#define CMD_MONITOR_SET_BUYER_REQ 0x955
//#define CMD_MONITOR_SET_BUYER_ACK 0x956
//#define CMD_MONITOR_SET_BUYER_IND CMD_SET_BUYER_REQ
...
/*vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv开始监控vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv*/
...
typedef struct XT_MONITOR_START_ACK //主管获取客服所接待的买家
{
	uint8      ret;
	uint32     idSeller;//客服ID
...
/*vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv结束监控vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv*/
...
/*vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv主管插入会话vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv*/
...
/*vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv主管接手会话vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv*/
...
typedef struct {
	uint32 teamid;
	uint32 userid;
	uint8  ReceptionState;
}customer_t;

//服务器使用
typedef struct XT_SERVER_CS_GET_ACK
{
	uint8   ret;	
	uint32 m_nID;				//当前clientservice的ID号
	char m_CSname[MAX_USERNAME_LEN + 1];	
	int m_State;
	int m_MaxRecConsult;		//最大咨询接待量		
	int m_permission;	
	XT_SERVER_CS_GET_ACK()
	{
		memset(this,0,sizeof(XT_SERVER_CS_GET_ACK));
	}
} XT_SERVER_CS_GET_ACK, *LPXT_SERVER_CS_GET_ACK;



typedef struct XT_WEB_QUEUE_LENGTH_IND
{
	uint8 ret;
	uint32 QueueId;//队列号
	uint32 QueueLength;//排队总长度
	struct WebQueueInd
	{
		uint32 UserId;
		uint32 UserPos;		
	}webQueueInd[1];
}XT_WEB_QUEUE_LENGTH_IND,*LPXT_WEB_QUEUE_LENGTH_IND;



typedef struct XT_JSON_STRING
{
	uint8 type;
	char jstring[8192];
}XT_JSON_STRING,*LPXT_JSON_STRING;

typedef struct SubAccountInf
{
	uint32  id;
	char	strEmail[MAX_EMAIL_LEN+1] = "";
	char	strUsername[MAX_USERNAME_LEN+1] = "";	// 用户名
...
typedef struct multiDevice_t
{
	uint32 userId;
	uint32 flag;
	uint32 deviceId;
	uint32 heartBeatTime;
	uint32 loginType;    // 1 PC  2 web  3 xmpp  4 html5
	uint32 clientIp;
	uint16 clientPort;
	uint32 serverip;
	char   sessionKey[MAX_SESSIONKEY_LEN];			//Session Key
	char   p2pSessionKey[MAX_SESSIONKEY_LEN];		//p2p Session Key

	multiDevice_t()
	{
		memset(this,0,sizeof(multiDevice_t));
	}
}multiDevice_t;
#endif
